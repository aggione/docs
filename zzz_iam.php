...
//		if ($r > 10) {$s.=' hasmanyrows="1"';}
		...
		if (in_array($n,['tabella_cerca_attivi','tabella_cerca_sge'])){$s.='<th></th>';}
		...
		for($i=0; $i < $r; $i++){						// loop sulle righe
			...
			for($di=0; $di<count($d); $di++){	// loop sulle descrizioni delle colonne (gruppi)
				...
				if (($n=='lista_eccezioni' or $n=='lista_eccezioni_disable') and $di==0){
					$s.='<td>';
						if ($a['EP'][$i]==1) {
							$s.='<button class="btn btn-warning btn-sm btn-block" act="ne" cf="'.$a['COD_FISC'][$i].'" kr="'.$a['KR'][$i].'" dom="eczn_bottom" tb="e" n="'.$n.'">Modifica</button>';
							$s.='<button class="btn btn-danger btn-sm btn-block" act="de" cf="'.$a['COD_FISC'][$i].'" nc="'.$a['NOME'][$i].' '.$a['COGNOME'][$i].'" dom="mm" n="'.$n.'">Cancella</button>';
						} else {
...
// if ($sviluppo_albo){write_log('_tmp.log',"get_ra_row - sql: ".$sql);}
		...
		$b=array(
			...
			, 'ura'=>false 				// modifica richiesta account
			, 'dra'=>false 				// cancella richiesta account
...
function get_cerca(){
	...
	if (empty($_REQUEST['ocerca'])){$_REQUEST['ocerca']=array('attivi');}

	// $l=array('attivi','scaduti','ldap','guest','sge','esse3','affiliati','eccezioni');
	$l=array(
	'attivi'=>"select a.*, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr from c##sss_import.ab_can a where ".$sw." order by a.cognome, a.nome, a.cod_fisc"
...
function get_delete_eccezioni(){
	...
	if (isset($_REQUEST['conferma'])){
		...
		$stl='bg-danger text-center text-white';
		$btn='<button id="modal-ok" class="btn btn-success btn-block" data-dismiss="modal" cf="'.trim($_REQUEST['cf']).'" n="'.$_REQUEST['n'].'" nc="'.$_REQUEST['nc'].'" act="de" dom="mm" conferma="y">Conferma cancellazione</button><button id="modal-ko" class="btn btn-dark btn-block" data-dismiss="modal">Annulla</button>';
		$domf='';
		$domc='';
	}
	$r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl, 'btn' => $btn, 'domf' => $domf, 'domc' => $domc);
...
function get_attivita(){
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza '.$ta.'</h4><strong>'.$sql.'</strong>','warning');
		// if (in_array($_SESSION['IAM']['uid_login'],array('a.bongiorni'))){$s.=get_alert($sql);}
...
function get_ict_adt(){ 									// allievi da trasferire
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute di allievi ed ex allievi (attualmente non attivi)</h4><strong>'.$sql.'</strong>','warning');
		// if (in_array($_SESSION['IAM']['uid_login'],array('a.bongiorni'))){$s.=get_alert($sql);}
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function get_ict_ude(){ 									// utenti da eliminare
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute di utenti non allievi o ex allievi (attualmente non attivi)</h4><strong>'.$sql.'</strong>','warning');
		// if (in_array($_SESSION['IAM']['uid_login'],array('a.bongiorni'))){$s.=get_alert($sql);}
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function get_anomalie_ab_can_ldap(){ 			// Anagrafiche non agganciate ad LDAP
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza i record in AB_CAN che non hanno il loro CF in un account LDAP</h4><div class="text-primary">il flag <strong>f_ldap</strong> in <strong>AB_RUOLO_NEW</strong> determina se l\'account di <strong>AB_CAN</strong> dovrebbe avere l\'account in LDAP</div><strong>'.$sql.'</strong>','warning');
		// if ($_SESSION['IAM']['uid_login'] == 'a.bongiorni' and $_SESSION['IAM']['devel']==1){$s.=get_alert($sql);}
		// $s.='<br />'.get_table_data($a[2],$g,$d,$nt);
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function get_ruoli_non_gestiti(){ 				// ruoli non gestiti in AB_RUOLO_NEW
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h2>Ruoli non gestiti in AB_RUOLO_NEW - trovati '.count($a[2]['LDAP_UID']).' records</h2>','info');
		$s.=get_alert('<h4>Questa ricerca visualizza i ruoli gestiti nei gestionali e mappati in AB_RUOLO_NEW</h4><strong>'.$sql.'</strong>','warning');
		// if ($_SESSION['IAM']['uid_login'] == 'a.bongiorni' and $_SESSION['IAM']['devel']==1){$s.=get_alert($sql);}
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function get_stesso_anno_nascita(){ 			// stesso anno di nascita in AB_CAN
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza i record in AB_CAN che hanno pi&ugrave; di un record per lo stesso Luogo, data di nascita e sesso (i crt dal 7&deg; al 15&deg;) per rilevare possibili duplicazioni</h4><strong>'.$sql.'</strong>','warning');
		// if ($_SESSION['IAM']['uid_login'] == 'a.bongiorni' and $_SESSION['IAM']['devel']==1){$s.=get_alert($sql);}
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function anomalie_caratteri_ldap(){				// ricerca anomalie caratteri in LDAP
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza i record in LDAP che hanno caratteri non standard nei campi sn, givenname, ou, mail</h4><strong>'.$sql.'</strong>','warning');
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
...
function dett_conta(){
	...
	$s='';
	// $s=get_alert(json_encode($_REQUEST));
	$sql="select ldap_uid, cognome, nome, mail, dr, da, di, gest from c##sss_import.ab_can where ".$_REQUEST['t']."='".$_REQUEST['k']."' order by cognome, nome";
	// if ($_SESSION['IAM']['uid_login'] == 'a.bongiorni' and $_SESSION['IAM']['devel']==1){
		// $s.=get_alert($sql);
	// }
	$a=load_db($conn_new,$sql,'o');
...
function get_ldap_da_disabilitare(){
	...
	// $sql.=" union ";
	$sql.="select ".$campiLDAP." from c##sss_import.anagrafiche_da_ldap, (select distinct cod_fisc from c##sss_import.ab_can) a where cf is not null and cf = cod_fisc(+) and cod_fisc is null";
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute</h4><strong>'.$sql.'</strong>','warning');
		$s.='<br />';
		// $s.='<div class="row">';
			// $s.='<div class="col-sm-3">'.get_table_data($a[2],'','',$nt,'o',false).'</div>';	
			// $s.='<div class="col-sm-9" id="contaR"></div>';		// numero righe
		// $s.='</div>';	
...
function get_can_d($uid=''){
	...
	$i=array_search($uid,$_SESSION['IAM']['ab_can']['LDAP_UID']);	// trova il primo
		...
		$s.='<div class="row py-2">'; // intestazione
			...
			$s.='<div class="col-sm-10">';
				$s.='<div class="row">';								
					...
					$ss=htmlentities($_SESSION['IAM']['ab_can']['NOME'][$i].' '.$_SESSION['IAM']['ab_can']['COGNOME'][$i]);
					$s.='<div class="col-sm-9"><h3>'.$ss.'</h3></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Gestionale:</div>';
					$ss=$_SESSION['IAM']['ab_can']['GEST'][$i];
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Uid:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['LDAP_UID'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Codice fiscale:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['COD_FISC'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Mail:</div>';
					$ss=''; if(!empty($_SESSION['IAM']['ab_can']['MAIL'][$i])){$ss=htmlentities($_SESSION['IAM']['ab_can']['MAIL'][$i]);}
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Telefono interno:</div>';
					$ss=''; if(!empty($_SESSION['IAM']['ab_can']['TEL'][$i])){$ss=htmlentities($_SESSION['IAM']['ab_can']['TEL'][$i]);}
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Ruolo:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['KR'][$i].' - '.$_SESSION['IAM']['ab_can']['DR'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Afferenza:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['KA'][$i].' - '.$_SESSION['IAM']['ab_can']['DA'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Inquadramento:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['KI'][$i].' - '.$_SESSION['IAM']['ab_can']['DI'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Fine rapporto:</div>';
					$ss=htmlentities($_SESSION['IAM']['ab_can']['DTF'][$i]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Nota:</div>';
					$ss=''; if(!empty($_SESSION['IAM']['ab_can']['NOTA'][$i])){$ss=htmlentities($_SESSION['IAM']['ab_can']['NOTA'][$i]);}
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
			$s.='</div>';
		$s.='</div>';
