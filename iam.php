...
/*	
	...
	TO DO:
		...
		- introdurre lo stato di da_cancellare
			deve essere elaborato su AD e passare in cancellato_ad (o disattivato_ad)
			dallo stato di ad passare a cancellato o disattivato (agendo su LDAP)

	oci_error($conn) Array Description	https://www.php.net/manual/en/function.oci-error.php
...
account: portineria.ceiic 				pwd: !2013pOrtineriacEiic?
account: portineria.toscanelli 		pwd: P0rt1n3r14T 						verificata 13.09.2021
account: portpsav 								pwd: !2013pOrtineriapSav?
account: portineria.daf 					pwd: !2013pOrtineriadAf?
account: Gestione Aule 						pwd: !hsfET67!=p 						Vecchia Password:g23SKDvmsW
account: portineria.sede 					pwd: kK3Tn4v3
account: portineria.terzani 			pwd: !4GwZ9932?
account: portineria.toscanelli 		pwd: P0rt1n3r14T
		...
		switch ($_REQUEST['func']) {
		 ...
		 // ----------------------------------------------------------------------------
			...
			case 'get_ra':				// richiesta account
				$s=get_ra();
				break;
			case 'ra_stato':			// rilettura richieste account pendenti o intero storico
				$s=get_ra_stato();	
				break;
			...
			// case 'imp_old':				// visualizza il VECCHIO repository degli account registrati in LDAP
				// $s=get_imp_old();
				// break;
			case 'mod_carma':				// lista dei dati comuni dei caricamenti massivi
				$s=mod_carma();
				break;
			case 'dett_carma':			// Dettaglio dei dati comuni dei caricamenti massivi
				$s=dett_carma();
				break;
			case 'sav_mod_carma':			// Salva i dati comuni dei caricamenti massivi
				$s=sav_mod_carma();
				break;
			case 'cerca_na':
			...
			case 'cf_calcola':		// 
				$calc = new CalcolaCF();
				$s=get_alert('CF calcolato: ' . $calc->calcola($_REQUEST['eczn_nome'], $_REQUEST['eczn_cognome'], $_REQUEST['eczn_genere'], new \DateTime(convert_date($_REQUEST['eczn_dtn'])), $_REQUEST['eczn_comune']),'warning'); // eczn_nome,eczn_cognome,eczn_genere,eczn_dtn,eczn_comune
				break;
			...
			case 'ict_cerca':
				switch ($_REQUEST['ict_azione']) {
					...
					case "cpr":			$s=get_conta_per_ruolo();						break;
					case "cpa":			$s=get_conta_per_afferenza();				break;
					case "cpi":			$s=get_conta_per_inquadramento();		break;
					case "cyall":		$s=get_cyberguru_all();							break;
					case "cdp":			$s=get_cyberguru_doc_pta();					break;
					case "cal":			$s=get_cyberguru_allievi();					break;
					case "ldapdd":	$s=get_ldap_da_disabilitare();			break;
				}
				break;
			case "dett_conta":
				$s=dett_conta();
...
if (true){		
			...
			$s.='<div class="row">';
				...
				$s.='<div class="col-sm-1">';
					...
					// }
					if ($su or $ict){
						$s.='<button title="Modifica i dati comuni dei caricamenti massivi" class="btn btn-info btn-sm btn-block" act="mod_carma" dom="ga_bottom">Mod. Car. Mass.</button>';
					}
				$s.='</div>';
				$s.='<div class="col-sm-1">';
				...
				$s.='<div class="col-sm-2">';
					...
					if ($su){
						$s.='<div class="row">';
							$s.='<div class="col-sm">';
								$s.='<button class="btn btn-info btn-sm btn-block" act="cfla" dom="mm" tipo="adc" title="Upload CSV account da creare">Upload CSV <i class="fa fa-upload" aria-hidden="true"></i></button>';
							$s.='</div>';
							$s.='<div class="col-sm">';
								$s.=' <button class="btn btn-warning btn-sm btn-block" act="cfla_alumni" dom="mm" tipo="alu" title="Upload CSV Alumni">Upload alum. <i class="fa fa-upload" aria-hidden="true"></i></button>';
							$s.='</div>';
						$s.='</div>';
					}
				$s.='</div>';
...
// if ($sviluppo_albo){write_log('_tmp.log',"get_tab - dopo get_ra_stato");}
		...
		case 'cerca':	
if (true){		
				$l=array('ab_can','scaduti','ldap','guest','sge','esse3','pre_immatricolati','affiliati','eccezioni','personale');
				$s.='<div class="row">';
					$s.='<div class="col-sm">';
						$s.='<input type="text" name="fcerca" id="fcerca" class="form-control form-control-sm" placeholder="UID o Codice fiscale o Nome o Cognome o Telefono" bta="bt_cerca" />';
					$s.='</div>';
					$s.='<div class="col-sm">';
						$s.='<select title="opzioni di ricerca" multiple="multiple" ID="ocerca" name="ocerca[]" class="form-control" data-actions-box="true" data-live-search="true" data-size="8" sep>';
							foreach ($l as $li){
								$s.='<option value="'.$li.'">'.strtoupper($li).'</option>';
							}
							// $s.='<option value="scaduti">SCADUTI</option>';
							// // $s.='<option value="communigate">anche communigate</option>';
							// $s.='<option value="ab_can">AB_CAN</option>';
							// $s.='<option value="ldap">LDAP</option>';
							// $s.='<option value="guest">LDAP guest</option>';
							// $s.='<option value="sge">SGE</option>';
							// $s.='<option value="esse3">ESSE3</option>';
							// $s.='<option value="affiliati">AFFILIATI</option>';
							// $s.='<option value="eccezioni">ECCEZIONI</option>';
						$s.='</select>';
					$s.='</div>';
					$s.='<div class="col-sm-2">';
						$s.='<button class="btn btn-primary btn-sm btn-block" act="get_cerca" dom="cerca_bottom" getf="ocerca,fcerca" id="bt_cerca">Cerca</button>';
					$s.='</div>';
				$s.='</div>';
			$s.='<div id="cerca_bottom" class="py-2"></div>';			
			break;
}			
		case 'ldap_ad':	
if (true){		
...
if (true){		
			...
			$s.='<div class="row">';
				...
				$s.='</div>';
				$s.='<div class="col-sm-2">';
					$s.='<button class="btn btn-danger btn-sm btn-block" act="me" edis="y" dom="eczn_bottom">Lista eccezioni disable</button>';
				$s.='</div>';
				$s.='<div class="col-sm-2">';
...
if (true){		
			...
			$s.='<form id="f_pu">';
				...
				$s.='<div class="row py-1">';
					...
					$s.='<div class="col-sm-6" id="col_ugov1">';
							...
							for ($j=0; $j < count($_SESSION['IAM']['utenti_ugov']['LDAP_UID']); $j++) {
								if ($_SESSION['IAM']['utenti_ugov']['LDAP_UID'][$j]!=''){
									$y=array_search($_SESSION['IAM']['utenti_ugov']['LDAP_UID'][$j],$_SESSION['IAM']['ab_can']['LDAP_UID']);
									$da=''; if ($y !== false){$da=' ('.$_SESSION['IAM']['ab_can']['DA'][$y].')';}
					...
					$s.='</div>';
					$s.='<div class="col-sm-2">';
						$s.='<button class="btn btn-dark btn-sm btn-block" act="can_ugov" dom="pu_bottom">Stato utenze UGOV delle carriere attive</button>';
					$s.='</div>';
				$s.='</div>';
				$s.='<div class="row py-1">';
					$s.='<div class="col-sm-2 text-right">';
			...
			$s.='<div class="row py-1">';
				$s.='<div class="col-sm-4">';
					$s.='<select name="ict_azione" id="ict_azione" class="form-control form-control-sm" value="" data-live-search="true" data-size="8" sep>';
						...
						$s.='<option data-tokens="cpr" value="cpr">AB_CAN - conta per ruolo</option>';
						$s.='<option data-tokens="cpa" value="cpa">AB_CAN - conta per afferenza</option>';
						$s.='<option data-tokens="cpi" value="cpi">AB_CAN - conta per inquadramento</option>';
						$s.='<option data-tokens="cyall" value="cyall">cyberguru Tutti</option>';
						$s.='<option data-tokens="cdp" value="cdp">cyberguru Docenti e PTA</option>';
						$s.='<option data-tokens="cal" value="cal">cyberguru Allievi</option>';
						$s.='<option data-tokens="ldapdd" value="ldapdd">LDAP da disabilitare</option>';
					$s.='</select>';
				$s.='</div>';
				$s.='<div class="col-sm-2">';
...
//		if ($r > 10) {$s.=' hasmanyrows="1"';}
		...
		if ($n=='lista_eccezioni' or $n=='lista_eccezioni_disable'){$s.='<th></th>';}
		// if ($n=='subscribers_ml'){$s.='<th>AD</th><th></th>';}
		if ($n=='subscribers_ml'){$s.='<th>AD</th>';}
		if ($n=='lista_attivita'){$s.='<th></th>';}
		if (in_array($n,['tabella_cerca_ab_can','tabella_cerca_sge'])){$s.='<th></th>';}
		...
		for($i=0; $i < $r; $i++){						// loop sulle righe
			...
			for($di=0; $di<count($d); $di++){	// loop sulle descrizioni delle colonne (gruppi)
				...
				if ($n=='tabella_cerca_ab_can'){
					if ($di==0){
						$s.='<td><button class="btn btn-outline-success btn-sm" title="Carriera attiva" dom="mm" act="dett_can" uid="'.$a['LDAP_UID'][$i].'"><i class="far fa-folder-open"></i></button></td>';
					}
				}
				if ($n=='tabella_cerca_sge'){
					if ($di==0){
						$s.='<td><button class="btn btn-outline-success btn-sm" title="record SGE" dom="mm" act="dett_sge" id_sge="'.$a['ID_SGE'][$i].'"><i class="far fa-folder-open"></i></button></td>';
					}
				}
				if ($n=='anomalie_ldap'){
					if ($di==0){
				...
				if (($n=='lista_eccezioni' or $n=='lista_eccezioni_disable') and $di==0){
					$s.='<td>';
						if ($a['EP'][$i]==1) { // eccezione presente
							$s.='<button class="btn btn-warning btn-sm btn-block" act="ne" rid="'.$a['RID'][$i].'" cf="'.$a['COD_FISC'][$i].'" kr="'.$a['KR'][$i].'" dom="eczn_bottom" tb="e" n="'.$n.'">Modifica</button>';
							$s.='<button class="btn btn-danger btn-sm btn-block" act="de" rid="'.$a['RID'][$i].'" cf="'.$a['COD_FISC'][$i].'" nc="'.$a['NOME'][$i].' '.$a['COGNOME'][$i].'" dom="mm" n="'.$n.'">Cancella</button>';
						} else { // eccezione non presente
...
*/
				...
				if ($n=='lista_attivita' and $di==0){
					$s.='<td>';
						$s.='<button title="toggle" class="btn btn-secondary btn-sm" act="tg_att" attivita="'.$a['ATTIVITA'][$i].'" dom="mm" gett="ATT_'.$a['ATTIVITA'][$i].'_F_ATTIVO">toggle</button>';
						if (!empty($a['N_IN_SGE'][$i])){
							$s.=' <button title="vedi" class="btn btn-info btn-sm" act="dett_sge_att" attivita="'.$a['ATTIVITA'][$i].'" dom="mm">vedi SGE</button>';
						}
					$s.='</td>';
				}
				if (in_array($n,["ce_aree","ce_moduli","ce_funzioni","ce_gruppi","ce_profili","ce_contesti","ce_ruoli","vi_gruppi","vi_profili","vi_contesti","vi_ruoli","co_gruppi","co_profili","co_contesti","co_ruoli"]) and $di==0){
					...
					if (!empty($a['KR'][$i])){
						$s.='<button title="visualizza dettaglio" class="btn btn-secondary btn-sm" act="visdett_pu" tt0="'.$tt[0].'" tt1="'.$tt[1].'" dom="mm" k="'.$a['KR'][$i].'">D</button>';
				...
				for($gi=0; $gi<count($g[$di]); $gi++){	// loop sui campi del gruppo
					...
					$title=$nc;
					if ($t=='o') {
						if (!empty($a[$nc][$i])){$tmp=htmlentities($a[$nc][$i]);} else {$tmp='';}

						if ($n=='conta_per_ruolo' or $n=='conta_per_afferenza' or $n=='conta_per_inquadramento'){
							if ($di==0 and $gi==0){
								$tmp='<button class="btn btn-outline-success btn-sm" title="dettaglio" dom="contaR" act="dett_conta" k="'.$a[$nc][$i].'" t="'.$nc.'">'.$tmp.'</button>';
							}
						}
					...
					if ($nc=='DFR'){
						$oggi=date('Y/m/d');
						if ($tmp < $oggi){
							$s.=get_alert('<span title="'.$title.'">'.$tmp.'</span>','danger',true);
						} else {
							$s.=get_alert('<span title="'.$title.'">'.$tmp.'</span>','success',true);
						}
					} else {
...
function get_dizionari($force=false){
	global $conn_new;
	if (empty($_SESSION['IAM']['lista_ruoli']) or $force){
	...
	if (empty($_SESSION['IAM']['lista_afferenze']) or $force){
	...
	if (empty($_SESSION['IAM']['tbmailist']) or $force){
	...
	if (empty($_SESSION['IAM']['ab_can']) or $force){
	...
	if (empty($_SESSION['IAM']['ab_csn']) or $force){
	...
	if (empty($_SESSION['IAM']['lista_comuni']) or $force){	// comuni
	...
	if (empty($_SESSION['IAM']['utenti_ugov']) or $force){
	...
	if (empty($_SESSION['IAM']['cmg_bkp']) or $force){
...
function aau_alu(){												// dopo upload file csv degli account alumni
...
function aau_adc(){												// dopo upload file csv degli account
	...
	for ($y=1; $y<count($csv) and $y < ($nmr + 1); $y++){	// loop sulle righe dati del csv (comincia da 1 ... la prima riga sono le intestazioni)
		...
		if ($ee==''){
			...
			$nome=crt_filter(ab_str_utf8_ascii(strtolower($csv[$y][$inome])),'§mi');	// pone in minuscolo - filtra solo lettere minuscole
			$cognome=crt_filter(ab_str_utf8_ascii(strtolower($csv[$y][$icognome])),'§mi');	// pone in minuscolo - filtra solo lettere minuscole
		}
	...
	if (!empty($out)){
		...
		for ($r=1; $r<count($out); $r++){
			...
			$sql.=",'".crt_filter(trim(ucwords(strtolower(trim(ab_str_utf8_ascii($out[$r][$inome]))))),'§ma§mi§nu. ',true)."'";		// ra_nome
			$icognome=array_search($cognome_exist[0],$cols);
			$sql.=",'".crt_filter(trim(ucwords(strtolower(trim(ab_str_utf8_ascii($out[$r][$icognome]))))),'§ma§mi§nu. ',true)."'";	// ra_cognome
			...
			if (!empty($kad)){
				...
				if ($ika !== false){
					$da="'".trim(substr(implode("",explode("'",ab_str_utf8_ascii($_SESSION['IAM']['lista_afferenze']['DA'][$ika]))),0,60))."'";	// tolgo eventuali ' (non previsti in AD)
				}
			}
			$sql.=",".$da;
...
function get_pdf_account($row){						// crea il pdf dell'account creato
	//	$row	= 	riga della tabella richieste_account
...
function get_pdf_account_guest($row){ 		// @@ non in produzione
	//	$row	= 	riga della tabella richieste_account
	...
	if (!empty($_REQUEST['conferma'])){
		...
		switch ($_REQUEST['type']){
			case "fra":
				...
				$tit.='Forza forward account';
				if (empty($e)){
					$msg.='Il froward dell\' account di <strong>'.$nome.' '.$cognome.'</strong> &egrave; stata correttamente eseguito';
					$stl.='bg-warning text-center text-white';
				} else {
					$msg.='Il froward dell\' account di <strong>'.$nome.' '.$cognome.'</strong> ha dato errore';
					$msg.=$e;
					$stl.='bg-danger text-center text-white';
				}
				break;
				...
				*/
				...
				if (empty($e)){
					if (!empty($_REQUEST['k'])){
						...
						if ($_REQUEST['tipo']=='PF'){
							$sql.=" ra_stato='aspetta_ad'";																			// ra_stato (prosegue creando AD e poi LDAP)
						} else {
							$sql.=" ra_stato='forza_ldap'";																			// ra_stato (prosegue creando solo LDAP)
						}
			...
			case "ura":	//	Modifica richiesta account
				...
				if (empty($e)){
					...
					if (empty($_REQUEST['k'])){
						...
						if (!empty($_REQUEST['ra_tipo'])){if ($_REQUEST['ra_tipo'] == 'PF'){
						...
						$sql.=",'".input_to_db($_REQUEST['nome'],true)."'";		// ra_nome
						// $sql.=",'".addslashes(ucwords(strtolower(trim(ab_str_utf8_ascii($_REQUEST['cognome'])))))."'";	// ra_cognome
						$sql.=",'".input_to_db($_REQUEST['cognome'],true)."'";	// ra_cognome
						$sql.=",'".trim(crt_filter(strtoupper($_REQUEST['cf']),'§ma§nu'))."'";	// ra_cf
						$sql.=",'".trim(crt_filter(strtolower($_REQUEST['email_ex']),'§mi§nu.-_@'))."'";						// ra_mail_notifica
						$sql.=",'".$_REQUEST['kr']."'";								// ra_ruolo
						$sql.=",'".$_REQUEST['ka']."'";								// ra_afferenza
						$da='';	// descrizione afferenza (necessaria per la creazione in AD)
						$i=array_search($_REQUEST['ka'],$_SESSION['IAM']['lista_afferenze']['KA']);
						if ($i !== false){$da=substr(implode("",explode("'",ab_str_utf8_ascii($_SESSION['IAM']['lista_afferenze']['DA'][$i]))),0,64);} // tolgo eventuali ' (non previsti in AD)
						...
						$sql.=",'".input_to_db($ncr)."'"; 										// ra_nc_referente
						$sql.=",'bozza'"; 											// ra_stato
						$sql.=",'".input_to_db($_REQUEST['note'])."'"; 							// ra_note
						...
						$sql.=", ra_nome='".input_to_db($_REQUEST['nome'],true)."'";
						// $sql.=", ra_cognome='".addslashes(ucwords(strtolower(trim(ab_str_utf8_ascii($_REQUEST['cognome'])))))."'";
						$sql.=", ra_cognome='".input_to_db($_REQUEST['cognome'],true)."'";
						$sql.=", ra_cf='".trim(crt_filter(strtoupper($_REQUEST['cf']),'§ma§nu'))."'";
						$sql.=", ra_mail_notifica='".trim(crt_filter(strtolower($_REQUEST['email_ex']),'§mi§nu.-_@'))."'";
						$sql.=", ra_ruolo='".$_REQUEST['kr']."'";
						$sql.=", ra_afferenza='".$_REQUEST['ka']."'";
						$da='';	// descrizione afferenza (necessaria per la creazione in AD)
						$i=array_search($_REQUEST['ka'],$_SESSION['IAM']['lista_afferenze']['KA']);
						if ($i !== false){$da=substr(implode("",explode("'",ab_str_utf8_ascii($_SESSION['IAM']['lista_afferenze']['DA'][$i]))),0,64);}
						$sql.=", ra_d_afferenza='".$da."'";
						$sql.=", ra_inizio='".convert_date($_REQUEST['di'])."'";
						$sql.=", ra_fine='".convert_date($_REQUEST['df'])."'";
						$sql.=", ra_cf_referente='".trim(crt_filter(strtoupper($_REQUEST['docint']),'§ma§nu'))."'";
						$sql.=", ra_nc_referente='".input_to_db($ncr)."'";
						$sql.=", ra_note='".input_to_db($_REQUEST['note'])."'";
						$sql.=", ra_usr_mod='".$_SESSION['IAM']['uid_login']."'";
						if (($su or $ict) and !empty($_REQUEST['ra_stato'])){
							$sql.=", ra_stato = '".$_REQUEST['ra_stato']."'";
						} 
						if ($su){
...
// ra_k, ra_tipo, ra_nome, ra_cognome, ra_mail, ra_cf, ra_mail_notifica, ra_notificata, ra_dt_notifica, ra_ruolo, ra_afferenza, ra_d_afferenza, ra_inquadramento, ra_inizio, ra_nuovo_inizio, ra_fine, ra_cf_referente, ra_nc_referente, ra_stato, ra_motivo, ra_uid, ra_aliases, ra_redirect, ra_prima_password, ra_modifica_password, ra_cancellabile, ra_note, ra_nfcm, ra_ldap_dn_ini, ra_ad_motivo, ra_ad_princ_mail, ra_ad_dn_ini, ra_usr_ins, ra_dt_ins, ra_ip_ins, ra_usr_mod, ra_dt_mod, ra_ip_mod							
							...
							foreach ($asu as $asui){
								...
								if (in_array($asui,array('ra_mail','ra_aliases','ra_ldap_dn_ini','ra_ad_princ_mail','ra_ad_dn_ini'))){
									if (trim($_REQUEST[$asui]) != ''){
										$sql.=", ".$asui."='".$_REQUEST[$asui]."'";
									}
									$asuitf=false;
								...
								if ($asuitf) {
									...
									$sql.=", ".$asui."='".input_to_db($_REQUEST[$asui])."'";	// accetta anche stringa vuota
								} 
							}
						}
						$sql.=" where ra_k=".$_REQUEST['k'];
...
// if ($sviluppo_albo){write_log('_tmp.log',"URA - sql: ".$sql);}
					...
					$ss = "ura - ".(empty($_REQUEST['k'])?'insert':'update')." - tipo = ".$_REQUEST['tipo_account'].", nc = ".input_to_db($_REQUEST['nome'],true)." ".input_to_db($_REQUEST['cognome'],true).", cf = ".trim($_REQUEST['cf']).", mail notifica = ".$_REQUEST['email_ex'].", inizio = ".$_REQUEST['di'].", fine = ".$_REQUEST['df'].", stato = ".(empty($_REQUEST['ra_stato'])?'bozza':$_REQUEST['ra_stato']).(empty($ncr)?', referente = '.input_to_db($ncr):'');
					...
					if (empty($e)){
						...
						$msg.='Il tentativo di salvare la richiesta di account di <strong>'.$nome.' '.$cognome.'</strong> ha dato errore';
						$msg.=$e;
						$stl.='bg-danger text-center text-white';
					}
				}
...
// if ($sviluppo_albo){write_log('_tmp.log',"YRA - sql: ".$sql);}
			...
			case "mra":	//	Invia per email la richiesta account
				...
				if (count($x) <= 0) {	// dati non trovati
					...
					if (empty($x[0]['ra_ad_princ_mail']) and $x[0]['ra_tipo']=='PF'){
				...
				if (empty($e)){
					...
					if ($x[0]['ra_tipo']=='PF'){
						...
						$messaggio.='<br /><strong>'.$x[0]['ra_uid'].'</strong>'; // uid
					}
...
//				$stl.='bg-secondary text-center text-white';
		...
		switch ($_REQUEST['type']){
			case "fra":	//	Ricrea forward account
				$tit.='Crea forward account';
				$msg.='Confermi la creazione del forward dell\'account di <strong>'.$nome.' '.$cognome.'</strong> ?';
				$smg.='('.''.')';
				$stl.='bg-warning text-center text-white';
				$btn='<button id="modal-ok" class="btn btn-success" data-dismiss="modal" act="get_ra" dom="mm" type="'.$_REQUEST['type'].'" k="'.$_REQUEST['k'].'" conferma="y">Conferma</button><button id="modal-ko" class="btn btn-dark" data-dismiss="modal">Annulla</button>';
				break;
			case "era":	//	Elabora richiesta account
				...
				break;
			case "ura":	//	Modifica richiesta account
				$tit.='Modifica richiesta account';
...
// if ($sviluppo_albo){write_log('_tmp.log',"Modifica bozza richiesta account - fine");}
			...
			case "lra":	// log richiesta account
				...
				$sql="select ral_d, ral_stato, ral_usr_ins, ral_dt_ins, ral_ip from storico_ldap.richieste_account_log where ral_ra_k=".$_REQUEST['k'];
				$x=array();	
				$msg='';
				$a=mysqli_query($conn_sl,$sql);
				if (empty(mysqli_error($conn_sl))){
...
function dett_ldap_ad($k,$tipo){					// visualizza il dettaglio da LDAP o AD
	...
	file_put_contents('_debug.txt',"--------------- \n", LOCK_EX);
	chmod('_debug.txt', 0777);	
	$s='';
	$m='';
	...
	file_put_contents('_debug.txt',count($j)." righe\n".$sql."\n", FILE_APPEND | LOCK_EX);
	if (count($j) > 0){
		$uid=$j[0]['ra_uid'];
		if ($devel){
		...
		file_put_contents('_debug.txt',"---> ".json_encode($o,JSON_PRETTY_PRINT)."\n", FILE_APPEND | LOCK_EX);
...
// if ($sviluppo_albo){write_log('_tmp.log',"get_ra_row - uid: $uid, password: $pp, tipo: 'LDAP', stato: $stato");}		
		...
		if ($o_ldap['esiste']==0 and $j[0]['ra_aliases'] != ''){
			...
			foreach($aa as $aai){
				$o_ldap=ldap_authenticate($aai,$pp,'LDAP');
		...
		if ($j[0]['ra_tipo'] == 'PF'){
			// se PF controllo congruenza tra LDAP e AD (generici e GUEST sono solo in LDAP)
			// if (($o_ldap['esiste']==1 and $o_ad['esiste']==0) or ($o_ldap['esiste']==0 and $o_ad['esiste']==1)){
			if ($o_ldap['esiste'] != $o_ad['esiste']){
				...
				if ($o_ldap['autenticato'] != $o_ad['autenticato']){
		...
		if ($o_ldap['autenticato']==1 and $o_ad['autenticato']==1 and !in_array($j[0]['ra_stato'],array('storicizzata','completata'))){
			...
			if ($su or $ict){
				$j[0]['note_elaborazione'].=get_alert('STATO INCONGRUO -> (esistente ed autenticato sia LDAP che AD) dovrebbe essere <strong>completata o storicizzata</strong>','danger',true);
				$congruente=false;
//  			} else {
//  				try {
...
//  			}
		...
		if (($o_ldap['esiste']==0 or $o_ad['esiste']==0) and in_array($j[0]['ra_stato'],array('storicizzata','completata')) and $j[0]['ra_tipo'] == 'PF'){
		...
		if ($j[0]['ra_stato']=='aspetta_ldap_cgp' and !empty($p) and !empty($uidcp) and $o_ldap['esiste']==1){
			$j[0]['note_elaborazione'].=get_alert('cambio password LDAP','warning');
			// prima di cambiare la password su LDAP riprovo la password nuova su AD
			$o1_ad=ldap_authenticate($uidcp,$p,'AD');
			if ($o1_ad['autenticato']==1){
				// lo script AD ha cambiato la password e funziona, cambio la password su LDAP
				$info=array();
				...
				if ($e==''){
					// riprovo l'autenticazione su LDAP dopo il cambio password
		...
		if (
			...
			$info=array();
			// 
			if ($j[0]['ra_tipo'] == 'PF'){$dn='uid='.$uid.',ou=Users,o=sss,c=it';}
			if ($j[0]['ra_tipo'] == 'GE'){$dn='uid='.$uid.',ou=Users,o=sss,c=it';}
			if ($j[0]['ra_tipo'] == 'GU'){$dn='uid='.$uid.',ou=GuestUsers,o=sss,c=it';}
			// $info["objectClass"][]="eduPerson";
...
//			$a=getNextSerial("ou=GuestUsers,o=sss,c=it","x-seriale"); // cattura next guest
			...
			*/
			...
			if ($j[0]['ra_tipo'] == 'GU'){$info["gidNumber"]='514';}
			...
			if ($j[0]['ra_tipo'] == 'PF'){
				if (!empty($j[0]['ra_mail'])){$info["mail"]=strtolower($j[0]['ra_mail']);} // mail esterna
				if (!empty($j[0]['ra_ad_princ_mail'])){$info["mail"]=strtolower($j[0]['ra_ad_princ_mail']);}	// mail santannapisa.it
				// $info["telephonenumber"]=$$j[0]['ldap_telephonenumber'];
				// $info["mobile"]=$$j[0]['ldap_mobile'];
				$info["x-codiceFiscale"]=$j[0]['ra_cf'];
			}

			if (!empty($j[0]['ra_afferenza'])){$info["departmentnumber"]=$j[0]['ra_afferenza'];}
			if (!empty($j[0]['ra_d_afferenza'])){$info["ou"]=$j[0]['ra_d_afferenza'];}
			if (!empty($j[0]['ra_ruolo'])){
				$info["businesscategory"]=$j[0]['ra_ruolo'];
				$info["schacPersonalPosition"]=array($j[0]['ra_ruolo']);	// AB 05/08/2024
			}
...
// if ($sviluppo_albo){write_log('_tmp.log',"get_ra_row - sql: ".$sql);}
						...
						// crea log completamento
						$e=set_ra_log($j[0]['ra_k'],"completa con scrittura su LDAP",'completata'); // scrivo il log						
					} catch (Exception $ee) {
						if ($ee!=''){$e.='<br />Impossibile aggiornare Mysql - '.$ee;}
					}
				} else {
			...
			if ($e!=''){
				...
				// if (!empty($j[0]['ra_ad_princ_mail'])){
				if ($j[0]['ra_tipo'] == 'PF' and !empty($j[0]['ra_ad_princ_mail'])){
					// reset prende il primo elemento dell'array
					$c='ABCreaForwarder '.strtolower(reset(explode('@',$j[0]['ra_ad_princ_mail'])));
					shell_exec('/usr/bin/perl AB.pl '.$c);
				}
			}
		...
		if ($o_ldap['autenticato']==1 and $o_ldap['esiste']==1){
			$j[0]['note_elaborazione'].=get_alert('<li>esiste in LDAP con password originale</li>','success',true);
		}
		if ($o_ldap['autenticato']==0 and $o_ldap['esiste']==1){
			$j[0]['note_elaborazione'].=get_alert('<li>esiste in LDAP con password cambiata</li>','warning',true);
			// if ($su and !empty($o_ldap['err'])){$j[0]['note_elaborazione'].=$o_ldap['err'].'<br />';}
		}
		if ($o_ldap['esiste']==0){
			$j[0]['note_elaborazione'].=get_alert('<li>NON esiste in LDAP</li>','danger',true);
			// if ($su and !empty($o_ldap['err'])){$j[0]['note_elaborazione'].=$o_ldap['err'].'<br />';}
		}
		if ($j[0]['ra_tipo'] == 'PF'){
			// i controlli su AD li faccio solo in caso di PF
			if ($o_ad['autenticato']==1 and $o_ad['esiste']==1){
				$j[0]['note_elaborazione'].=get_alert('<li>esiste in AD con password'.($adt?' temporanea</li>':' originale'),'success',true);
			}
			if ($o_ad['autenticato']==0 and $o_ad['esiste']==1){
				$j[0]['note_elaborazione'].=get_alert('<li>esiste in AD con password cambiata</li>','warning',true);
				// if ($su and !empty($o_ad['err'])){$j[0]['note_elaborazione'].=$o_ad['err'].'<br />';}
			}
			if ($o_ad['esiste']==0){
				$j[0]['note_elaborazione'].=get_alert('<li>NON esiste in AD</li>','danger',true);
				// if ($su and !empty($o_ad['err'])){$j[0]['note_elaborazione'].=$o_ad['err'].'<br />';}
			}
		}
		
		// in caso di account "completato", inserito da almeno 10 giorni, e AD e LDAP esistenti 
		// e entrambi con password modificata, devono passare in storicizzato
		if ($o_ldap['autenticato']==0 and $o_ldap['esiste']==1 and $o_ad['autenticato']==0 and $o_ad['esiste']==1 and $j[0]['ra_tipo'] == 'PF' and $j[0]['ra_stato']=='completata'){
			$dia=substr($j[0]['ra_dt_ins'],0,10);				// data inserimento
			$ngf=date("Y-m-d",strtotime("-10 days"));		// 10 giorni fà
			if ($dia < $ngf){
				// $j[0]['note_elaborazione'].=get_alert('<li>STORICIZZARE</li>','primary',true);
				try {
					$sql="update storico_ldap.richieste_account set ra_stato = 'storicizzata' where ra_k = ".$j[0]['ra_k'];
					$a=mysqli_query($conn_sl,$sql);						
					if (!empty(mysqli_error($conn_sl))){$j[0]['note_elaborazione'].=mysqli_error($conn_sl);}
					$j[0]['ra_stato']='storicizzata';
					$j[0]['note_elaborazione'].=get_alert('<li>STORICIZZATA</li>','primary',true);
					// crea log completamento
					$e=set_ra_log($j[0]['ra_k'],"storicizzata in automatico",'storicizzata'); // scrivo il log
				} catch (Exception $ee) {
					if ($ee!=''){$e.='<br />Impossibile aggiornare Mysql - '.$ee;}
				}
				if ($e!=''){$j[0]['note_elaborazione'].=$e;}
			}
		}
		
		// array dei bottoni già creati (per evitare duplicazioni)
		$b=array(
			  'cra'=>false 				// conferma richiesta account
			, 'ura'=>false 				// modifica richiesta account (update)
			, 'dra'=>false 				// cancella richiesta account (delete)
			, 'yra'=>false 				// valida ICT
		...
		if ($j[0]['ra_stato']=='rifiutata'){	// @@@@ non ancora implementato
			if (!$b['ura']){$s.='<button class="btn btn-outline-info btn-sm" title="modifica richiesta account" dom="mm" type="ura" act="get_ra" k="'.$j[0]['ra_k'].'"><i class="fa fa-pen"></i></button>'; $b['ura']=true;}
		...
		if ($j[0]['ra_stato']=='completata' and $o_ldap['esiste']==1){
			if ($j[0]['ra_tipo']!='PF' or $o_ad['esiste']==1){
		...
		if (in_array($j[0]['ra_stato'],array('completata','storicizzata')) and $o_ldap['esiste']==1 and $o_ad['esiste']==1 and $j[0]['ra_tipo']=='PF' and !empty($j[0]['ra_ad_princ_mail'])){
			// tento di leggere il forward
			$tofo=explode('@',$j[0]['ra_ad_princ_mail']);
			$fo = shell_exec('/usr/bin/perl AB.pl ABGetForwarder '.strtolower($tofo[0]));
			if (empty($fo) or $fo != strtolower($tofo[0]).'@alumnisssup.mail.onmicrosoft.com'){
				// se non esiste o se errato visualizzo il pulsante per la creazione
				if (!$b['fra']){$s.='<button class="btn btn-warning btn-sm" title="forza forward account" dom="mm" type="fra" act="get_ra" k="'.$j[0]['ra_k'].'"><i class="far fa-share-square"></i></button>'; $b['fra']=true;}
				$j[0]['note_elaborazione'].=get_alert('<li>Forward mancante o errato</li>','danger',true);
			} else {
				$j[0]['note_elaborazione'].=get_alert('<li>Forward corretto</li>','success',true);
			}
		}
...
// if ($sviluppo_albo){write_log('_tmp.log',"get_ra_stato - count: ".count($j));}
	...
	if ($su or $ict) {
		$s.=get_alert('trovati '.count($j).' records'.$m,'secondary py-2 text-center');
	}
	if ($su or $ict) {
		// controllo se ci sono record in aspetta_ad da oltre 10 minuti
		$dmf = new Datetime('NOW', new DateTimeZone('Europe/Rome'));
		$dmf->add(DateInterval::createFromDateString('-10 minutes'));
		$sdmf=$dmf->format("Y-m-d H:i:s");
		// $sql="select * from storico_ldap.richieste_account where ra_stato='aspetta_ad' and ra_dt_mod is not null and ra_dt_mod < STR_TO_DATE('".$sdmf."', '%Y-%m-%d %H:%i:%s')";
		$sql="select * from storico_ldap.richieste_account where ra_stato = 'aspetta_ad' and TIMESTAMPDIFF(MINUTE, ra_dt_mod, now()) > 10";
		$b=mysqli_query($conn_sl,$sql);
		if (!empty(mysqli_error($conn_sl))){
			$s.='<br />'.mysqli_error($conn_sl);
		}
		if (mysqli_num_rows($b) > 0) {	// dati trovati
			$s.=get_alert('<h3>Batch AD probabilmente non funzionante</h3>','danger py-1 text-center');
		}
	}
						...
						$s.='<div class="row bg-light py-1">';
							...
							$s.='<div class="col-sm-8">';
								if (!empty($j[0][$asui])){
									$v=$j[0][$asui];
									if (in_array($asui,array('ra_dt_notifica','ra_nuovo_inizio'))){
										$v=convert_date($v,'yyyy-mm-dd','dd/mm/yyyy');
									}
								} else {
									$v='';
								}
...
function get_e_ra(){											// completa le informazioni per l'account (calcolo uid)
	...
	if ($j[0]['ra_tipo']=='GU'){	// guest account
		...
		if (empty($j[0]['ra_afferenza'])){$e.=get_alert('<strong>Struttura</strong> obbligatoria','danger');}
		if (empty($j[0]['ra_ruolo'])){$e.=get_alert('<strong>Categoria</strong> obbligatoria','danger');}
		if (empty($j[0]['ra_note'])){$e.=get_alert('<strong>Inserisci la descrizione dell\'evento/attivit&agrave; nelle note</strong> obbligatorio','danger');}
				...
				$s.='<div class="col-sm-8">';
					...
					$s.='<div class="row bg-light">';
						...
						$s.='<div class="col-sm-5">';
							...
							if (!empty($_SESSION['IAM']['my_acl']['ldap_ad'])){
								if ($_SESSION['IAM']['my_acl']['ldap_ad']!='S'){
									$s.=' disabled';
								}
							} else {$s.=' disabled';}
							$s.=' />';
...
function mod_carma(){											// Modifica i dati comuni dei caricamenti massivi
	global $conn_sl;
	$s='';

	$s.=get_alert('Visualizza lista accaount e modifica i dati comuni dei caricamenti massivi','info text-center');
	
	$s.='<div class="row bg-light">';
		$s.='<div class="col-sm-6">';
	
		$j=array();
		$sql="select distinct ra_nfcm, ra_dt_ins from storico_ldap.richieste_account where IFNULL(trim(ra_nfcm),'') <> '' order by ra_dt_ins desc";
		$a=mysqli_query($conn_sl,$sql);
		if (mysqli_num_rows($a) > 0) {	// dati trovati
			foreach($a as $row){$j[]=$row;}
			...
			foreach($j as $row){
				$s.='<tr>';
					$s.='<td><button class="btn btn-info btn-sm btn-block" act="dett_carma" nfcm="'.$row['ra_nfcm'].'">Dettaglio</button></td>';
					$s.='<td>'.str_replace('(','<strong>(',str_replace(')',')</strong>',$row['ra_nfcm'])).'</td>';
					$s.='<td>'.$row['ra_dt_ins'].'</td>';
				$s.='</tr>';
			}
			$s.='</tbody></table>';
		}
		...
		$s.='</div>';
		$s.='<div id="dett_carma" class="col-sm-6"></div>';
	$s.='</div>';
	$s.='<div id="dett_carma_rows"></div>';

	return $s;
}
function dett_carma(){										// Dettaglio (lista account) dei caricamenti massivi
	global $conn_sl;
	...
	$s.=get_alert(str_replace('(','<strong>(',str_replace(')',')</strong>',$_REQUEST['nfcm'])),'warning text-center');	// nome file caricamento massivo

	$j=array();
	$sql="select * from storico_ldap.richieste_account where ra_nfcm = '".$_REQUEST['nfcm']."'";
	$a=mysqli_query($conn_sl,$sql);
	if (mysqli_num_rows($a) > 0) {	// dati trovati
		foreach($a as $row){$j[]=$row;}
	} else {
		$r=array();
		$m=get_alert('Nessuna richiesta associata a questa importazione massiva - Segnala il problema all\'amministratore del sistema','danger');
		$r['domm'][0]['domf']='dett_carma';				$r['domm'][0]['domc']=$m;
		return json_encode($r);
	}

	$row=$j[0];	// primo account da cui prendo i dati comuni
	// if ($su){$m.=get_alert(json_encode($row));}
	
	/* $row
		{
				"ra_k": "586891",
				"ra_tipo": "PF",
				"ra_nome": "Ruth Kefelew",
				"ra_cognome": "Abera",
				"ra_mail": null,
				"ra_cf": "BRARHK98P51Z315P",
				"ra_mail_notifica": "kefelewruth@gmail.com",
				"ra_notificata": "0",
				"ra_dt_notifica": null,
				"ra_ruolo": "MAG",
				"ra_d_ruolo": null,
				"ra_afferenza": "005733",
				"ra_d_afferenza": "U.O. PhD-Lauree Magistrali",
				"ra_inquadramento": null,
				"ra_inizio": "2024-02-19",
				"ra_nuovo_inizio": null,
				"ra_fine": "2024-04-19",
				"ra_cf_referente": "",
				"ra_nc_referente": "",
				"ra_stato": "completata",
				"ra_motivo": null,
				"ra_uid": "r.abera",
				"ra_aliases": "Ruth.Abera",
				"ra_redirect": null,
				"ra_prima_password": "2EnszqcS",
				"ra_modifica_password": null,
				"ra_uid_modifica_password": null,
				"ra_cancellabile": "1",
				"ra_note": "files\/Bionics_imm_2023_2023mod.csv",
				"ra_nfcm": "inserito tramite caricamento csv (Bionics_imm_2023_2023mod.csv)",
				"ra_ldap_dn_ini": "uid=r.abera,ou=Users,o=sss,c=it",
				"ra_ad_motivo": null,
				"ra_ad_princ_mail": "Ruth.Abera@santannapisa.it",
				"ra_ad_dn_ini": "CN=Ruth Kefelew Abera,OU=U.O. PhD-Lauree Magistrali,OU=OpenLdap,DC=sssapisa,DC=it",
				"ra_usr_ins": "a.pellegrini",
				"ra_dt_ins": "2024-02-19 11:14:50",
				"ra_ip_ins": null,
				"ra_usr_mod": "a.pellegrini",
				"ra_dt_mod": "2024-02-19 11:51:15",
				"ra_ip_mod": null
		}	
	*/
	
	$m.='<form id="f_mod_csv_default">';
		$m.='<div class="row bg-light py-1">';
			$m.='<div class="col-sm-4 text-right">Struttura (Department):</div>';
			$m.='<div class="col-sm-8">';
				$ss='';
				if (!empty($_SESSION['IAM']['lista_afferenze'])){
					for ($y=0; $y < count($_SESSION['IAM']['lista_afferenze']['K']); $y++) {
						$v=$_SESSION['IAM']['lista_afferenze']['K'][$y];
						$ss.='<option data-tokens="'.$v.'" value="'.$v.'"';
						if ($v == $row['ra_afferenza']){$ss.=' selected';}
						$ss.='>'.$_SESSION['IAM']['lista_afferenze']['D'][$y].'</option>';
					}
				}
				$m.='<select name="ka" id="ka" class="form-control form-control-sm" value="" data-live-search="true" data-size="8" sep ><option data-tokens="0" value="0"></option>'.$ss.'</select>';
		...
		$m.='<div class="row bg-light py-1">';
			...
			$m.='<div class="col-sm-8">';
				...
				if (!empty($_SESSION['IAM']['lista_ruoli'])){
					for ($y=0; $y < count($_SESSION['IAM']['lista_ruoli']['K']); $y++) {
						$v=$_SESSION['IAM']['lista_ruoli']['K'][$y];
						$ss.='<option data-tokens="'.$v.'" value="'.$v.'"';
						if ($v == $row['ra_ruolo']){$ss.=' selected';}
						$ss.='>'.$_SESSION['IAM']['lista_ruoli']['D'][$y].'</option>';
					}
				}
				$m.='<select name="kr" id="kr" class="form-control form-control-sm" value="" data-live-search="true" data-size="8" sep ><option data-tokens="0" value="0"></option>'.$ss.'</select>';
		...
		$m.='<div class="row bg-light py-1">';
			...
			$m.='<div class="col-sm-8">';
				$d=date("d/m/Y");
				if (!empty($row['ra_inizio'])){$d=convert_date($row['ra_inizio'],'yyyy-mm-dd','dd/mm/yyyy');}
				$m.='<input type="text" name="di" id="di" class="form-control form-control-sm" value="'.$d.'" dto />';
			$m.='</div>';
		$m.='</div>';
		$m.='<div class="row bg-light py-1">';
			$m.='<div class="col-sm-4 text-right">Data fine (end date):</div>';
			$m.='<div class="col-sm-8">';
				$nd=date('Y-m-d', strtotime('+60 days'));
				$d=date_format(date_create($nd),"d/m/Y");
				if (!empty($row['ra_fine'])){$d=convert_date($row['ra_fine'],'yyyy-mm-dd','dd/mm/yyyy');}
				$m.='<input type="text" name="df" id="df" class="form-control form-control-sm" value="'.$d.'" dto />';
			$m.='</div>';
		$m.='</div>';
		$m.='<div class="row bg-light py-1">';
			$m.='<div class="col-sm-4 text-right">Referente Interno (Internal Referee):</div>';
				...
				if (!empty($_SESSION['IAM']['ab_can'])){
					...
					for ($y=0; $y < count($a['COD_FISC']); $y++) {
						if (in_array($a['KR'][$y],$ram1) or (in_array($a['KR'][$y],$ram2) and $a['FL_RESPONSABILE'][$y] == 1)){
							...
							if ($a['KR'][$y] == 'DC' or $a['KR'][$y] == 'ND' ){$bg='warning';}
							$ss.='<option class="alert-'.$bg.'" data-tokens="'.$a['COD_FISC'][$y].'" value="'.$a['COD_FISC'][$y].'"';
							if ($a['COD_FISC'][$y] == $row['ra_cf_referente']){$ss.=' selected';}
							$ss.='>'.$a['COGNOME'][$y].' '.$a['NOME'][$y].' ('.$a['COD_FISC'][$y].')</option>';
						}
					}
				}
		...
		// if ($su or $ict){
			$m.='<div class="row bg-light py-1">';
				...
				$m.='<div class="col-sm-8">';
					...
					$ass=array();
					$ass["bozza"]="Bozza";
					$ass["richiesta"]="Richiesta";
					$ass["validata_ict"]="Validata ICT";
					$ass["completata"]="Completata";
					if ($row['ra_stato'] == 'storicizza'){$ass['storicizzata']='Storicizzata';}
					foreach($ass as $k => $v){
						$ss.='<option data-tokens="'.$k.'" value="'.$k.'"';
						if ($k == $row['ra_stato']){$ss.=' selected';}
						$ss.='>'.$v.'</option>';
					}
					// $ss.='<option data-tokens="bozza" value="bozza">Bozza</option>';
					// $ss.='<option data-tokens="richiesta" value="richiesta">Richiesta</option>';
					// $ss.='<option data-tokens="validata_ict" value="validata_ict">Validata ICT</option>';
					// $ss.='<option data-tokens="completata" value="completata">Completata</option>';
					$m.='<select name="stato" id="stato" class="form-control form-control-sm" value="" data-live-search="true" data-size="8" sep ><option data-tokens="0" value="0"></option>'.$ss.'</select>';
				$m.='</div>';
			$m.='</div><br />';
		// }
	...
	$m.='</form>';
	$m.='<button class="btn btn-info btn-sm btn-block" act="sav_mod_carma" nfcm="'.$row['ra_nfcm'].'" tab="f_mod_csv_default" dom="mm">Salva le modifiche ai dati comuni degli account di questo caricamento massivo</button>';

	$s.=$m;
	$sd=$s;


// tipo	nome	cognome	cf	d_afferenza	inizio	fine	stato	uid	aliases	prima_password	usr_ins	dt_ins	elab				
// ra_k ra_tipo ra_nome ra_cognome ra_mail ra_cf ra_mail_notifica ra_notificata ra_dt_notifica ra_ruolo ra_d_ruolo ra_afferenza ra_d_afferenza ra_inquadramento ra_inizio ra_nuovo_inizio ra_fine ra_cf_referente ra_nc_referente ra_stato ra_motivo ra_uid ra_aliases ra_redirect ra_prima_password ra_modifica_password ra_uid_modifica_password ra_cancellabile ra_note ra_nfcm ra_ldap_dn_ini ra_ad_motivo ra_ad_princ_mail ra_ad_dn_ini ra_usr_ins ra_dt_ins ra_ip_ins ra_usr_mod ra_dt_mod ra_ip_mod 
	
	$s=get_alert('Trovati '.count($j).' account');
	$s.='<div class="row bg-light">';
		$s.='<div class="col-sm">';
			$s.='<table class="table table-striped table-sm table-responsive" id="tb-old-account"><thead>';
				$s.='<tr>';
					$s.='<th>Tipo</th>';
					$s.='<th>Nome e Cognome</th>';
					$s.='<th>CF</th>';
					$s.='<th>Struttura</th>';
					$s.='<th>Categoria</th>';
					$s.='<th>Inizio</th>';
					...
					$s.='<th>Uid</th>';
					$s.='<th>Operatore</th>';
					$s.='<th>Data inserimento</th>';
				$s.='</tr></thead><tbody>';
			foreach($j as $row){
				$s.='<tr>';
					$s.='<td>'.$row['ra_tipo'].'</td>';
					$s.='<td>'.$row['ra_nome'].' '.$row['ra_cognome'].'</td>';
					$s.='<td>'.$row['ra_cf'].'</td>';
					$s.='<td>'.$row['ra_afferenza'].' '.$row['ra_d_afferenza'].'</td>';
					$s.='<td>'.$row['ra_ruolo'].' '.$row['ra_d_ruolo'].'</td>';
					$s.='<td>'.$row['ra_inizio'].'</td>';
					$s.='<td>'.$row['ra_fine'].'</td>';
					$s.='<td>'.$row['ra_cf_referente'].' '.$row['ra_nc_referente'].'</td>';
					$s.='<td>'.$row['ra_stato'].'</td>';
					$s.='<td>'.$row['ra_uid'].'</td>';
					$s.='<td>'.$row['ra_usr_ins'].'</td>';
					$s.='<td>'.$row['ra_dt_ins'].'</td>';
				$s.='</tr>';
			}
			$s.='</tbody></table>';
	...
	$r=array();
	$r['domm'][0]['domf']='dett_carma';				$r['domm'][0]['domc']=$sd;
	$r['domm'][1]['domf']='dett_carma_rows';	$r['domm'][1]['domc']=$sb;
	return json_encode($r);
...
*/
function sav_mod_carma(){									// salva modifiche al caricamento massivo
	global $conn_sl;
	$s='';

	$s.=get_alert('Lista account caricamento massivo modificati<h3>'.$_REQUEST['nfcm'].'</h3>','info text-center');
	
	$j=array();
	$sql="select * from storico_ldap.richieste_account where ra_nfcm = '".$_REQUEST['nfcm']."' and ra_stato not in ('aspetta_ad','creato_ad','completata','storicizzato')";
	$a=mysqli_query($conn_sl,$sql);
	if (mysqli_num_rows($a) > 0) {	// dati trovati
	
		$sql="update storico_ldap.richieste_account set";
		$sql.=" ra_stato='".$_REQUEST['stato']."'";
		if (!empty($_REQUEST['ka'])){$sql.=", ra_afferenza='".$_REQUEST['ka']."'";}
		if (!empty($_REQUEST['kr'])){$sql.=", ra_ruolo='".$_REQUEST['kr']."'";}
		if (!empty($_REQUEST['di'])){$sql.=", ra_inizio='".convert_date($_REQUEST['di'])."'";}
		if (!empty($_REQUEST['df'])){$sql.=", ra_fine='".convert_date($_REQUEST['df'])."'";}
		if (!empty($_REQUEST['docint'])){$sql.=", ra_cf_referente='".$_REQUEST['docint']."'";}
		$sql.=" where ra_nfcm = '".$_REQUEST['nfcm']."'";
		$sql.=" and ra_stato not in ('aspetta_ad','creato_ad','completata','storicizzata')";
		
		$s.=get_alert($sql,'warning');
		$a=mysqli_query($conn_sl,$sql);
		$s.=get_alert('Sono stati modificati '.mysqli_affected_rows($conn_sl).' account','warning');
		...
		$s=get_alert('Non ci sono account da modificare','danger');
	}

	// $s.=get_alert('<pre>'.json_encode($_REQUEST,JSON_PRETTY_PRINT).'</pre>');
	$a=array();
	$a['tit']='Modifica dati caricamento massivo';
	$a['msg']=$s;
	$a['dom']='mm';
	$s=json_encode($a);
	return $s;
...
// --- cerca ---
function get_accordion_item($a,$g,$d,$t,$n,$saccn) {					//	
	$s='<div class="card">';
		$s.='<div id="card-header-'.$n.'" class="card-header">';
			$s.='<a class="card-link" data-toggle="collapse" href="#acc-'.$n.'">'.$t.'</a>';
		$s.='</div>';
		$s.='<div id="acc-'.$n.'" class="collapse" data-parent="#accordion'.$saccn.'">';
			$s.='<div class="card-body">';
				$s.=get_table_data($a,$g,$d,$n);
			$s.='</div>';
		$s.='</div>';
	$s.='</div>';
	return $s;
...
function get_cerca(){
	...
	if (empty($_REQUEST['fcerca'])){return get_alert('Inserisci un termine di ricerca','danger');}

	$s='';
	$f=$_REQUEST['fcerca'];
	$aa = explode('|', $f); for($i=0; $i<count($aa); $i++){$aa[$i]=strtoupper(trim($aa[$i]));}
	$cc = array("trim(upper(a.COGNOME))","trim(upper(a.NOME))","trim(upper(a.COGNOME || ' ' || a.NOME))","trim(upper(a.NOME || ' ' || a.COGNOME))","trim(upper(a.LDAP_UID))","trim(upper(a.COD_FISC))","trim(TO_CHAR(TEL))");
	$sw='';
	foreach ($aa as $aaa) {
		foreach ($cc as $ccc) {
			if (!$sw==''){$sw.=' or ';}
			$sw.=' '.$ccc." Like '%".str_replace("'","''",$aaa)."%'";
		}
	}

	if (empty($_REQUEST['ocerca'])){$_REQUEST['ocerca']=array('ab_can');}

	// $l=array('attivi','scaduti','ldap','guest','sge','esse3','affiliati','eccezioni');
	$l=array(
	'ab_can'=>"select a.*, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr from c##sss_import.ab_can a where ".$sw." order by a.cognome, a.nome, a.cod_fisc"
	,'scaduti'=>"select a.*, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr from c##sss_import.ab_csn a where ".$sw." order by a.cognome, a.nome, a.cod_fisc"
	,'ldap'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, NOTA, gest from (select LDAP_UID, MAIL, TELEPHONENUMBER || ' ' || MOBILE tel, CF cod_fisc, SN cognome, GIVENNAME nome, DEPARTMENTNUMBER ka, OUBREVE da, null dab, BUSINESSCATEGORY kr, EMPLOYEETYPE dr, null dtr, null peso_sssup, null ki, null di, to_date(substr(SCADENZATEMPO,1,8),'yyyymmdd') dt_rap_fin, SAMBAACCTFLAGS nota, null gest from c##sss_import.anagrafiche_da_ldap) a where ".$sw." order by cognome, nome,cod_fisc"
	,'guest'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, NOTA, gest from (select LDAP_UID, MAIL, TELEPHONENUMBER || ' ' || MOBILE tel, CF cod_fisc, SN cognome, GIVENNAME nome, DEPARTMENTNUMBER ka, OUBREVE da, null dab, BUSINESSCATEGORY kr, EMPLOYEETYPE dr, null dtr, null peso_sssup, null ki, null di, to_date(substr(SCADENZATEMPO,1,8),'yyyymmdd') dt_rap_fin, SAMBAACCTFLAGS nota, null gest from c##sss_import.anagrafiche_guest_da_ldap) a where ".$sw." order by cognome, nome,cod_fisc"
	,'sge'=>"select ID_SGE,LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select ID_SGE, null LDAP_UID, null MAIL, null tel, COD_FIS cod_fisc, cognome, nome, AFF_ORG ka, DS_AFF_ORG da, DS_AFF_ORG_BREVE dab, RUOLO kr, DS_RUOLO dr, DS_TIPO_RUOLO dtr, null peso_sssup, INQUADR ki, DS_INQUADR di, dt_rap_fin, 'id: ' || ID_SGE || ' - matricola: ' || MATRICOLA || ' - attivita: ' || ATTIVITA || ' ' || DS_ATTIVITA || ' ' || to_char(DECORR_ATT, 'yyyy/mm/dd') || ' - contratto: ' || ID_CONTRATTO  || ' (ins: ' || to_char(DT_INS, 'yyyy/mm/dd') || ' ' || USR_INS || ' - mod: ' || to_char(DT_MOD, 'yyyy/mm/dd') || ' ' || USR_MOD || ')' nota, 'UGOV' gest from c##sss_tables.v_ie_ru_sge) a where ".$sw." order by cognome, nome, cod_fisc"
	,'esse3'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select null LDAP_UID, EMAIL mail, null tel, COD_FIS cod_fisc, cognome, nome, CODICEDIP ka, DESDIP da, null dab, TIPO_CORSO_COD kr, TIPO_CORSO_DES dr, null dtr, null peso_sssup, CODICECORSO ki, DESCORSO di, nvl(trunc(DATA_CHIUSURA), to_date('22220202','yyyymmdd')) dt_rap_fin, MOTIVO nota, 'ESSE3' gest from c##sss_import.elenatt_tot) a where ".$sw." order by cognome, nome, cod_fisc"
	,'pre_immatricolati'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select * from C##SSS_IMPORT.ESSE3_PRE_IMM_AA_CORR) a where ".$sw." order by cognome, nome, cod_fisc"
	,'affiliati'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select null LDAP_UID, case when MAIL_SSSUP is not null then MAIL_SSSUP else MAIL_ESTERNA end mail, tel_esterno || ' ' || tel_sssup tel, CF cod_fisc, cognome, nome, IST_SSSUP ka, DISTITUTO da, null dab, TA kr, DTA dr, null dtr, null peso_sssup, TRANSLATE (INITCAP (TRIM (IST_PROV || ' ' || UO_IST_PROV)) ,'ABCDEFGHIJKLMNOPQRSTUVWXYZ abcdefghijklmnopqrstuvwxyz' ,'ABCDEFGHIJKLMNOPQRSTUVWXYZ') ki, IST_PROV di, nvl(trunc(DT_FINE), to_date('22220202','yyyymmdd')) dt_rap_fin, note || ' ' || pnota || ' (ins: ' || to_char(DT_INS, 'yyyy/mm/dd') || ' ' || USR_INS || ' - mod: ' || to_char(DT_MOD, 'yyyy/mm/dd') || ' ' || USR_MOD || ')' nota, 'AFF' gest from c##sss_affiliati.affiliati_ex) a where ".$sw." order by cognome, nome, cod_fisc"
	,'eccezioni'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select ECZN_UID ldap_uid, MAIL_ESTERNA mail, null tel, cod_fisc, cognome, nome, ka, da, null dab, kr, dr, dtr, null peso_sssup, ki, di, dt_rap_fin, nota, 'ECZN' gest from c##sss_import.carriere_eccezioni) a where ".$sw." order by cognome, nome, cod_fisc" 
	,'personale'=>"select LDAP_UID,MAIL,TEL,COD_FISC,COGNOME,NOME,KA,DA,DAB,KR,DR,DTR,PESO_SSSUP,KI,DI, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr, nota, gest from (select ID_AB, USER_NAME LDAP_UID, EMAIL MAIL, null tel, COD_FIS cod_fisc, cognome, nome, AFF_ORG ka, DS_AFF_ORG da, DS_AFF_ORG_BREVE dab, CD_RUOLO kr, DS_RUOLO dr, null dtr, null peso_sssup, INQUADR ki, null di, dt_rap_fin, 'id: ' || ID_AB || ' - matricola: ' || MATRICOLA nota, 'UGOV' gest from c##sss_tables.V_IE_RU_PERSONALE) a where ".$sw." order by cognome, nome, cod_fisc" 
	); 
// c##sss_tables.V_IE_RU_PERSONALE - ID_AB ,CD_ESTERNO ,MATRICOLA ,MATRICOLA_SP ,NOME ,COGNOME ,MIDDLE_NAME ,COD_FIS ,CD_GENERE ,DT_NASCITA ,ID_COMU_NASC ,DS_CITTSTRA_NASC ,CD_CATASTO_NASC ,CD_ISTAT_NASC ,DS_COMUNE_NASC ,DS_PROV_NASC ,CD_SIGLA_NASC ,ID_NAZIONE_NASC ,DS_NAZI_NASC ,DS_NAZI_BREVE_NASC ,NAZI_CD_ISTAT_NASC ,NAZI_CD_CATASTO ,NAZI_CD_ISO3166_1 ,NAZI_CD_ISO3166_1_A2 ,CD_ONORIFICO ,PREF_ONOR ,DS_ONORIFICO ,ID_COMU_FIS ,CD_CATASTO_FIS ,CD_ISTAT_FIS ,DS_COMUNE_FIS ,CD_CAP_FIS ,INDIRIZZO_FIS ,NUM_CIVICO_FIS ,CD_SIGLA_FIS ,ID_COMU_RES ,CD_CATASTO_RES ,CD_ISTAT_RES ,DS_COMUNE_RES ,CD_CAP_RES ,INDIRIZZO_RES ,NUM_CIVICO_RES ,CD_SIGLA_RES ,ID_COMU_DOM ,CD_CATASTO_DOM ,CD_ISTAT_DOM ,DS_COMUNE_DOM ,CD_CAP_DOM ,INDIRIZZO_DOM ,NUM_CIVICO_DOM ,CD_SIGLA_DOM ,CD_COMPARTO ,DS_COMPARTO ,CD_RUOLO ,DS_RUOLO ,DS_RUOLO_LOCALE ,PESO_RUOLO ,DT_RAP_INI ,DT_RAP_FIN ,CD_SSD ,DS_SSD ,CD_FAC ,DS_FAC ,AFF_ORG ,DS_AFF_ORG ,DS_AFF_ORG_BREVE ,AFF_ORG2 ,DS_AFF_ORG2 ,DS_AFF_ORG2_BREVE ,AFF_ORG3 ,DS_AFF_ORG3 ,DS_AFF_ORG3_BREVE ,AFF_ORG4 ,DS_AFF_ORG4 ,DS_AFF_ORG4_BREVE ,SEDE ,DS_SEDE ,DS_SEDE_BREVE ,PROFILO ,DS_PROFILO ,DS_PROFILO_BREVE ,CD_ATENEO_CORR ,DS_ATENEO_CORR ,DT_INI_RAP_UNIV ,TELRIF ,EMAIL ,URLCV ,FL_DOCENTE ,USER_NAME ,USER_EMAIL ,CD_ANAGRAFICO ,INQUADR ,BADGE ,TEMPO ,DATA_CSISSSUP

	foreach ($_REQUEST['ocerca'] as $i){
		if (in_array($i, array_keys($l)) !== false){
			// $sql="select a.*, to_char(a.DT_RAP_FIN,'yyyy/mm/dd') dfr from c##sss_import.ab_can a where ".$sw." order by a.cognome, a.nome";
			$sql=$l[$i];
			$a=load_db($conn_new,$sql,'o');
			if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
				// $s.=get_alert('<strong>'.strtoupper($i).'</strong> (trovate '.$a[1].' righe)','info text-center',true);
				$g=[['LDAP_UID','MAIL','TEL','COD_FISC'],['COGNOME','NOME'],['KA','DA','DAB'],['KR','DR','DTR','PESO_SSSUP'],['KI','DI'],['DFR'],['GEST','NOTA']];
				$d=['Uid mail tel CF','Cognome e nome','Afferenza','Ruolo','Inquadramento','Fine rapp.','gestionale nota'];
				$s.='<div id="accordion'.$i.'">';
					$s.=get_accordion_item($a[2],$g,$d,strtoupper($i).' - trovate <span class="badge badge-success">'.$a[1].'</span> righe','tabella_cerca_'.$i,$i);
				$s.='</div>';
			} else {
				$m='<h3>'.strtoupper($i).' - Nessun dato trovato</h3>';
				if ($su){$m.=$sql;}
				$s.=get_alert($m,'danger',true);
			}
		}
	}
	return $s;
...
function get_nuova_eccezione(){						// form
	...
	$upd=false;
	$tabella='carriere_eccezioni';
	if (!empty($_REQUEST['n'])){
		if ($_REQUEST['n']=='lista_eccezioni_disable'){
			$tabella='carriere_eccezioni_disable';
		}
	}
	if (!empty($_REQUEST['cf'])){
		// controllo se il CF esiste già in carriere_eccezioni
		$sql="select * from c##sss_import.".$tabella." where cod_fisc = '".trim($_REQUEST['cf'])."'";	
		$a=load_db($conn_new,$sql,'o');
		...
		if ($iu=='u'){$sql.=$tabella;} else {$sql.='ab_cn';}
...
//					}
			...
			$s.='<div class="row bg-light py-1">';
				...
				$s.='</div>';
			$s.='</div>';
			if (!empty($_SESSION['IAM']['my_acl']['eccezioni'])){
				if ($_SESSION['IAM']['my_acl']['eccezioni']=='S'){
					if ($iu=='u'){
...
function get_salva_nuova_eccezione() {
	...
	// controllo se il CF esiste già in carriere_eccezioni
	$sql="select * from ".$tabella." where cod_fisc = '".$_REQUEST['eczn_cf']."'";
	$a=load_db($conn_new,$sql,'o');
...
*/	
	...
	$nt='lista_eccezioni';
	if (!empty($_REQUEST['storico'])){$nt='lista_eccezioni_storico';}
	if (!empty($_REQUEST['edis'])){$nt='lista_eccezioni_disable';}
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$t='Eccezioni';
		if (!empty($_REQUEST['storico'])){$t='Storico eccezioni';}
		if (!empty($_REQUEST['edis'])){$t='Eccezioni Disable';}
		$s='<br />'.get_alert($t.' - Trovati '.$a[1].' record','info').get_table_data($a[2],$g,$d,$nt);
	} else {
...
function get_delete_eccezioni(){
	...
	if (isset($_REQUEST['conferma'])){
		$t='carriere_eccezioni';
		if (isset($_REQUEST['n'])){if ($_REQUEST['n']=='lista_eccezioni_disable'){$t='carriere_eccezioni_disable';}}
		$sql="delete from c##sss_import.".$t." where cod_fisc='".trim($_REQUEST['cf'])."'";
		$a=load_db($conn_new,$sql,'o');
		...
		$cd=''; 
		if (isset($_REQUEST['n'])){if ($_REQUEST['n']=='lista_eccezioni_disable'){$cd='disable';}}
		$msg="Confermi la cancellazione dell'eccezione ".$cd." per:<br /><strong>".$_REQUEST['nc']."</strong> (".trim($_REQUEST['cf']).") ?";
		$stl='bg-danger text-center text-white';
		$btn='<button id="modal-ok" class="btn btn-success btn-block" data-dismiss="modal" rid="'.$_REQUEST['rid'].'" cf="'.trim($_REQUEST['cf']).'" n="'.$_REQUEST['n'].'" nc="'.$_REQUEST['nc'].'" act="de" dom="mm" conferma="y">Conferma cancellazione</button><button id="modal-ko" class="btn btn-dark btn-block" data-dismiss="modal">Annulla</button>';
		$domf='';
		$domc='';
	}
	$r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl, 'btn' => $btn, 'domf' => $domf, 'domc' => $domc);
...
function get_table_richieste_foto(){
	...
	$l=scandir($dir);
	if (empty($l)){
		return '<br />'.get_alert('Nessuna richiesta da elaborare','danger');
	}
	$a = array_diff($l, array('..', '.'));		// tolgo . e ..
	foreach($a as $f){
	...
	if ($automatismo){
		...
		$s.='<form id="f_automatismo" class="alert alert-warning">';
			...
			$s.='<div class="row">';
				...
				$s.='<div class="col-sm-8">';
					$q1=''; 
					if (!empty($_SESSION['IAM']['tbmailist']['QUERY'][$indice])){
						$q1=implode("'",explode("''",$_SESSION['IAM']['tbmailist']['QUERY'][$indice]));
					}
					$s.='<textarea class="form-control form-control-sm" title="query per feed" placeholder="query per feed" id="QUERY" name="QUERY" rows="3">'.$q1.'</textarea>';
				$s.='</div>';
				$s.='<div class="col-sm-2">';
			...
			$s.='<div class="row">';
				...
				$s.='<div class="col-sm-8">';
					$q2=''; 
					if (!empty($_SESSION['IAM']['tbmailist']['QUERY2'][$indice])){
						$q2=implode("'",explode("''",$_SESSION['IAM']['tbmailist']['QUERY2'][$indice]));
					}
					$s.='<textarea class="form-control form-control-sm" title="query per null" placeholder="query per null" id="QUERY2" name="QUERY2" rows="3">'.$q2.'</textarea>';
				$s.='</div>';
				$s.='<div class="col-sm-2">';
...
//		}
		...
		$s.='<tr class="alert-'.$css.'">';
			...
			if (strpos($css,'success') !== false){
				$s.='<td><h4 class="text-'.$css.'">'.$tbmailist['NOME_LISTA'][$i].'</h4></td>';
				...
				$s.='<td class="text-'.$css.'">'.$tbmailist['NOME_LISTA'][$i].'</td>';
...
//	or_organigramma
		...
		if ($do){
			...
			if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
				if ($_REQUEST['tipo']=='2'){
					$c1 = 0; $c2=0;
					foreach ($a[2]['USER_NAME'] as $ai){if ($ai == $_REQUEST['ugov1']) {$c1++;} else {$c2++;}}
					$s.='<div class="row alert alert-'.$ta.' colnext"><h3>'.$rk[1].' - <span class="text-danger">'.$a[1].'</span> righe - ('.$_REQUEST['ugov1'].' <span class="text-danger">'.$c1.'</span>) ('.$_REQUEST['ugov2'].' <span class="text-danger">'.$c2.'</span>)</h3></div>';
				} else {
...
function rsql_pu() { 											// ricalcola sql per la ricerca
	...
	$w = "trunc(dt_inizio_val) <= trunc(sysdate) and trunc(dt_fine_val) >= trunc(sysdate)";
	$w .= " and CODFIS = COD_FISC(+)";
	if (!empty($cerca)){
		$w = " and (upper(USER_NAME) like '%" . strtoupper($cerca) . "%' or upper(DS_USER) like '%" . strtoupper($cerca) . "%')";
	}
...
function visdett_pu(){										// visualizza dettagli del ruolo
	global $conn_new;
	$tit='<h2>'.$_REQUEST['title'].'</h2>';
	$msg='';
	// {"func":"visute_pu","obj_val":"","title":"visualizza utenti del profilo","class":"btn btn-secondary btn-sm","act":"visute_pu","tt0":"ce","tt1":"profili","dom":"mm","k":"DI0005"}
	...
	$sql0="select * from (".$_SESSION['IAM']['asql_pu_ugov']['ce_ruoli'].") where ID_RUOLO='".$_REQUEST['k']."'";
	// utenti
	$sql1="select distinct user_name, case when cognome is null then ds_user else  cognome || ' ' || nome end cn, CODFIS cf, dr, da, to_char(dt_rap_fin,'dd/mm/yyyy') dt_rap_fin from C##SSS_IMPORT.AB_UTENZE_UGOV_DIRITTI_EX WHERE ID_RUOLO='".$_REQUEST['k']."' order by cn"; 
	// gruppi
	$sql2="select * from (".$_SESSION['IAM']['asql_pu_ugov']['ce_gruppi'].") where ID_GRP in (select distinct ID_GRP from c##sss_import.ab_fw01_diritti_ex where ID_RUOLO='".$_REQUEST['k']."')";
	// profili
	$sql3="select * from (".$_SESSION['IAM']['asql_pu_ugov']['ce_profili'].") where CD_PROFILO in (select distinct CD_PROFILO from c##sss_import.ab_fw01_diritti_ex where ID_RUOLO='".$_REQUEST['k']."')";

	$a=load_db($conn_new,$sql0,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$msg.=get_alert($_REQUEST['tt1'],'info colnext').'<div class="alert-secondary">'.get_table_data($a[2],'','','tab_ruo','o',false).'</div>';
	}
	$a=load_db($conn_new,$sql1,'o',9);
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$msg.=get_alert('utenti','info colnext').'<div>'.get_table_data($a[2],'','','tab_user','o',false).'</div>';
	}
	$a=load_db($conn_new,$sql2,'o',9);
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$msg.=get_alert('gruppi','info colnext').'<div>'.get_table_data($a[2],'','','tab_gruppi','o',false).'</div>';
	}
	$a=load_db($conn_new,$sql3,'o',9);
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$msg.=get_alert('profili','info colnext').'<div>'.get_table_data($a[2],'','','tab_profili','o',false).'</div>';
	}
...
function agg_pu(){ 												// aggiorna le viste necessarie alla ricerca
	...
	if (is_array($oe)){
		...
		$s2='<select name="ugov2" id="ugov2" class="form-control form-control-sm" data-live-search="true" data-size="8" sep >
			'.$ss2.'
		</select>';
...
function can_ugov(){
	...
	if (strpos($_SESSION['IAM']['my_acl']['permessi_ugov'],'S')!==false){$su=true;} else {$su=false;}	// superuser

	$sql="SELECT a.*, to_char(a.DT_RAP_FIN, 'yyyy/mm/dd') dfr, to_char(u.dt_inizio_val, 'yyyy/mm/dd') dt_inizio_val, to_char(u.dt_fine_val, 'yyyy/mm/dd') dt_fine_val FROM 	c##sss_import.ab_can a, c##sss_tables.fw01_user u WHERE a.ldap_uid = u.user_name(+) AND trunc(nvl(u.dt_inizio_val, to_date('19900101', 'yyyymmdd'))) <= trunc(sysdate) AND trunc(nvl(u.dt_fine_val, to_date('22220202', 'yyyymmdd'))) >= trunc(sysdate) ORDER BY a.cognome, a.nome, a.cod_fisc";
	// 1984 = primo ciclo phd
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$g=[['LDAP_UID','MAIL','TEL','COD_FISC'],['COGNOME','NOME'],['KA','DA','DAB'],['KR','DR','DTR','PESO_SSSUP'],['KI','DI'],['DFR'],['GEST','NOTA'],['DT_FINE_VAL']];
		$d=['Uid mail tel CF','Cognome e nome','Afferenza','Ruolo','Inquadramento','Fine rapp.','gestionale nota','utente ugov'];
		$s.=get_alert('<h3>CAN vs UGOV - trovate <span class="badge badge-success">'.$a[1].'</span> righe</h3>','success');
		$s.=get_table_data($a[2],$g,$d,'can_ugov');
	} else {
		$m='<h3>CAN vs UGOV  - Nessun dato trovato</h3>';
		if ($su){$m.=$sql;}
		$s.=get_alert($m,'danger',true);
	}
	return $s;	
...
function get_attivita(){
	...
	$sql="select ATTIVITA, DESCR, BREVE, to_char(DT_INS,'YYYY/MM/DD HH24:MI:SS') DT_INS, F_ATTIVO, NOTA from C##SSS_IMPORT.AB_ATTIVITA_SSS order by attivita";
	// --- lettura attività
	if (!empty($_REQUEST['tipo_attivita'])){if ($_REQUEST['tipo_attivita']=='u'){
		$ta='le attivit&agrave; usate';
		$sql="select a.ATTIVITA, a.DESCR, a.BREVE, to_char(a.DT_INS,'YYYY/MM/DD HH24:MI:SS') DT_INS, a.F_ATTIVO, a.NOTA, asge.N n_in_sge from C##SSS_IMPORT.AB_ATTIVITA_SSS a, (select attivita, count(attivita) n from C##SSS_TABLES.V_IE_RU_SGE group by attivita) asge where a.attivita = asge.attivita order by a.attivita";
	}} 
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='lista_attivita';
		$s.=get_alert('<h2>Attivit&agrave; - trovati '.$a[1].' records</h2>','info text-center');
		$s.=get_alert('<h4>Questa ricerca visualizza '.$ta.'</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
		}
...
function dett_sge_att(){
	global $conn_new;
	$s='';
	$tit='Dettaglio SGE con attivit&agrave; '.$_REQUEST['attivita'];
	$msg='';
	// $msg.=str_replace("\n","<br />",json_encode($_REQUEST,JSON_PRETTY_PRINT));
	$stl='bg-info text-center text-white';
	// $btn='';
	// $r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl, 'btn' => $btn, 'domf' => $domf, 'domc' => $domc);

	$campiSGE="ID_SGE, MATRICOLA, COD_FIS, COGNOME, NOME, TO_CHAR(DT_INI,'yyyy/mm/dd') DT_INI, TO_CHAR(DT_FIN,'yyyy/mm/dd') DT_FIN, TO_CHAR(DT_RAP_INI,'yyyy/mm/dd') DT_RAP_INI, TO_CHAR(DT_RAP_FIN,'yyyy/mm/dd') DT_RAP_FIN, RUOLO, DS_RUOLO, AFF_ORG, DS_AFF_ORG, ATTIVITA, DS_ATTIVITA, ID_CONTRATTO, TO_CHAR(DECORR_ATT,'yyyy/mm/dd') DECORR_ATT, TIPO_PART_TIME, DS_TIPO_PT, PERC_PT, USR_INS, USR_MOD, TO_CHAR(DT_INS,'yyyy/mm/dd HH24:MI') DT_INS, TO_CHAR(DT_MOD,'yyyy/mm/dd HH24:MI') DT_MOD";

	$g=[['ID_SGE','MATRICOLA','COD_FIS'],['COGNOME','NOME'],['DT_INI','DT_FIN'],['DT_RAP_INI','DT_RAP_FIN'],['RUOLO','DS_RUOLO'],['AFF_ORG','DS_AFF_ORG'],['ATTIVITA','DS_ATTIVITA','DECORR_ATT','ID_CONTRATTO'],['TIPO_PART_TIME','DS_TIPO_PT','PERC_PT'],['DT_INS','USR_INS','DT_MOD','USR_MOD']];
	$d=['Id Matricola CF','Cognome Nome','Date (INI,FIN)','Date (RAP_INI,RAP_FIN)','Ruolo','Afferenza','Attivit&agrave; decorrrenza contratto','part-time','ins mod'];

	$sql="select ".$campiSGE." from C##SSS_TABLES.V_IE_RU_SGE where attivita = '".$_REQUEST['attivita']."' ORDER BY COGNOME, NOME";
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$tit.=' ('.$a[1].' records)';
		$msg.='<br />'.get_table_data($a[2],$g,$d,'lista_sge_attivita');
	} else {
		$msg.=get_alert('Nessun record SGE con attivit&agrave; '.$_REQUEST['attivita'],'danger');
	}

	$r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl);
	$s=json_encode($r);			
	return $s;	
}
function dett_sge(){
	...
	// $btn='';
	// $r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl, 'btn' => $btn, 'domf' => $domf, 'domc' => $domc);
	if (empty($_REQUEST['id_sge'])){
		$msg.=get_alert('Record SGE non specificato','danger');
	} else {
		$sql="select * from C##SSS_TABLES.V_IE_RU_SGE where id_sge = ".$_REQUEST['id_sge'];
		$a=load_db($conn_new,$sql,'o');
		if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
			// $tit.=' ('.$a[1].' records)';
			// $msg.='<br />'.get_table_data($a[2],$g,$d,'lista_sge_attivita');
			foreach(array_keys($a[2]) as $i){
				$msg.='<div class="row"><div class="col-sm-3 text-right">'.$i.'</div><div class="col-sm"><strong>'.htmlentities($a[2][$i][0]).'</strong></div></div>';
			}
		} else {
			$msg.=get_alert('Record SGE non trovato','danger');
		}
	}
	$r=array('tit' => $tit, 'msg' => $msg, 'stl' => $stl);
	$s=json_encode($r);			
...
function get_ict_adt(){ 									// allievi da trasferire
	...
	$sql="SELECT A.LDAP_UID, a.sn cognome, a.givenname nome, a.CF, a.BUSINESSCATEGORY kr, a.EMPLOYEETYPE dr, a.DEPARTMENTNUMBER ka, a.OU da, a.oubreve dab, a.PHYSICALDELIVERYOFFICENAME du,
		TO_CHAR (TO_DATE (SUBSTR (A.SCADENZATEMPO,	1,	8),	'yyyymmdd'), 'yyyy/mm/dd') scadenza_ldap
	FROM C##SSS_IMPORT.ANAGRAFICHE_DA_LDAP a
	WHERE
		a.LDAP_UID NOT IN (SELECT DISTINCT ldap_uid FROM C##SSS_IMPORT.ab_can WHERE ldap_uid IS NOT NULL)
		AND a.cf NOT IN (SELECT DISTINCT cod_fisc FROM C##SSS_IMPORT.ab_can WHERE cod_fisc IS NOT NULL)
		AND (
			(A.CF IN (SELECT DISTINCT COD_FIS FROM C##SSS_IMPORT.ELENATT_TOT WHERE TIPO_CORSO_COD IN ('CO1L', 'CO2L', 'COCU5', 'COCU6', 'D2', 'D226')))
			OR 
			a.LDAP_UID IN (SELECT DISTINCT USER_NAME FROM	C##SSS_IMPORT.EX_ALLIEVI WHERE USER_NAME IS NOT NULL)
			)
		$w
	ORDER BY a.BUSINESSCATEGORY, a.sn, a.givenname 	
	";
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='allievi_da_trasferire';
		$s.=get_alert('<h2>Allievi da trasferire - trovati '.count($a[2]['LDAP_UID']).' records</h2>','info text-center');
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute di allievi ed ex allievi (attualmente non attivi)</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function get_ict_ude(){ 									// utenti da eliminare
	...
	$sql="SELECT a.LDAP_UID, a.CF, a.sn cognome, a.givenname nome,
		CASE WHEN b.cod_fis IS NOT NULL THEN 'possibile maternità' ELSE '' END maternita,
		a.BUSINESSCATEGORY kr, a.EMPLOYEETYPE dr, a.DEPARTMENTNUMBER ka, a.OU da, a.oubreve dab,
		a.PHYSICALDELIVERYOFFICENAME du,
		TO_CHAR (TO_DATE (SUBSTR (a.SCADENZATEMPO,1,8),'yyyymmdd'),'yyyy/mm/dd') scadenza_ldap
	FROM C##SSS_IMPORT.ANAGRAFICHE_DA_LDAP a,
	(SELECT * FROM c##sss_tables.v_ie_ru_sge
	WHERE attivita IN (SELECT DISTINCT attivita FROM ab_attivita_sss WHERE lower(descr) LIKE '%mater%')
	AND (trunc(dt_ini) >= trunc(sysdate) - 270 OR trunc(dt_rap_ini) >= trunc(sysdate) - 270)) b
	WHERE
		a.cf = b.cod_fis(+)
		AND a.LDAP_UID NOT IN (SELECT DISTINCT ldap_uid FROM C##SSS_IMPORT.ab_can WHERE ldap_uid IS NOT NULL)
		AND a.cf NOT IN (SELECT DISTINCT cod_fisc FROM C##SSS_IMPORT.ab_can WHERE cod_fisc IS NOT NULL)
		AND a.cf NOT IN (SELECT DISTINCT COD_FIS FROM C##SSS_IMPORT.ELENATT_TOT WHERE TIPO_CORSO_COD IN ('CO1L', 'CO2L', 'COCU5', 'COCU6', 'D2', 'D226'))
		AND a.ldap_UID NOT IN (SELECT DISTINCT USER_NAME FROM C##SSS_IMPORT.EX_ALLIEVI WHERE USER_NAME IS NOT NULL)
		$w
	ORDER BY a.BUSINESSCATEGORY, a.sn, a.givenname";	
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='utenti_da_eliminare';
		$s.=get_alert('<h2>Utenti da eliminare da LDAP - trovati '.count($a[2]['LDAP_UID']).' records</h2>','info text-center');
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute di utenti non allievi o ex allievi (attualmente non attivi)</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function get_anomalie_ab_can_ldap(){ 			// Anagrafiche non agganciate ad LDAP
	global $conn_new;
	$s='';
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h4>Questa ricerca visualizza i record in AB_CAN che non hanno il loro CF in un account LDAP</h4><div class="text-primary">il flag <strong>f_ldap</strong> in <strong>AB_RUOLO_NEW</strong> determina se l\'account di <strong>AB_CAN</strong> dovrebbe avere l\'account in LDAP</div>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function get_ruoli_non_gestiti(){ 				// ruoli non gestiti in AB_RUOLO_NEW
	...
	$s='';
	$sql="select 
eu.kr kr_gest, eu.dr dr_gest, eu.g gest, eu.n
...
from 
(select distinct g, kr, dr, n from (
        select 'ESSE3' g, tipo_corso_cod kr, tipo_corso_des dr, count(*) n from c##sss_import.elenatt_tot group by tipo_corso_cod, tipo_corso_des
        union
        select 'UGOV' g, ruolo kr, ds_ruolo dr, count(*) n from C##SSS_TABLES.V_IE_RU_SGE group by ruolo, ds_ruolo
        union
        select distinct 'AFF' g, ta kr, dta dr, count(*) n from C##SSS_AFFILIATI.periodi_ex group by ta, dta
        union
        select distinct 'ECZN' g, kr, dr, count(*) n from c##sss_import.CARRIERE_ECCEZIONI group by kr, dr
    )
) eu,
...
order by eu.dr";
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h2>Ruoli non gestiti in AB_RUOLO_NEW - trovati '.count($a[2]['KR_GEST']).' records</h2>','info');
		$s.=get_alert('<h4>Questa ricerca visualizza i ruoli gestiti nei gestionali e mappati in AB_RUOLO_NEW</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function get_stesso_anno_nascita(){ 			// stesso anno di nascita in AB_CAN
	global $conn_new;
	$s='';
	$campi=array('ldap_uid','mail','tel','cell','gest','k','id_ab','nome','cognome','cod_fisc','genere','dt_nascita','comune','mail_esterna','ka','da','dab','kr','dr','dtr','ki','di','kas','ks','ds','ku','du','tr','dt_rap_ini','dt_rap_fin','nota','peso_sssup','ruolo_solari','ruolo_allievi','ruolo_net7','groups_net7','groups_intranet','groups_net7_intranet','uo','uo_padre','rpath','dt_refresh');
	...
	$campi=array('ldap_uid','mail','gest','k','id_ab','nome','cognome','cod_fisc','genere',"to_char(dt_nascita,'dd/mm/yyyy') dt_nascita",'comune','da','dr','di','tr','dt_rap_fin','nota');
	$sql="select substr(x.cod_fisc,7,9) ddnn, ".implode(',',$campi)." from ab_can x where substr(cod_fisc,7,9) in (select ddnn from (select a.*, substr(a.cod_fisc,7,9) ddnn from ab_can a) group by ddnn having COUNT(*) > 1) order by substr(cod_fisc,7,9), cognome, nome";
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='stesso_anno_nascita';
		$s.=get_alert('<h2>stessa data e luogo di nascita in AB_CAN - trovati '.count($a[2]['LDAP_UID']).' records</h2>','info');
		$s.=get_alert('<h4>Questa ricerca visualizza i record in AB_CAN che hanno pi&ugrave; di un record per lo stesso Luogo, data di nascita e sesso (i crt dal 7&deg; al 15&deg;) per rilevare possibili duplicazioni</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function anomalie_caratteri_ldap(){				// ricerca anomalie caratteri in LDAP
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		...
		$s.=get_alert('<h2>Anomalie caratteri LDAP (sn || givenname) (ou) (mail)</strong> - trovati '.count($a[2]['LDAP_UID']).' records</h2>','info');
		$s.=get_alert('<h4>Questa ricerca visualizza i record in LDAP che hanno caratteri non standard nei campi sn, givenname, ou, mail</h4>','warning');
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
			}
...
function dett_conta(){
	...
	$s='';
	// $s=get_alert(json_encode($_REQUEST));
	$sql="select distinct ldap_uid, cognome, nome, mail, dr, da, di, gest from c##sss_import.ab_can where ".$_REQUEST['t']."='".$_REQUEST['k']."' order by cognome, nome";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='dettaglio_conta';
		$t='Conta';
		if ($_REQUEST['t']=='KR'){$t='Conta per ruolo';					$kd='DR';}
		if ($_REQUEST['t']=='KA'){$t='Conta per afferenza';			$kd='DA';}
		if ($_REQUEST['t']=='KI'){$t='Conta per inquadramento';	$kd='DI';}
		$d=$a[2][$kd][0];
		$s.=get_alert('<h2>'.$d.' ('.$a[1].' record)</h2>','info');
		$s.='<br />'.get_table_data($a[2],'','',$nt,'o');
	// } else {
		// $s.=get_alert('<h2>Nessun dato</h2>','danger text-center');
	}
	return $s;	
}
function get_conta_per_ruolo(){
	global $conn_new;
	$s='';
	$sql="select kr, dr, count(kr) n from c##sss_import.ab_can group by kr, dr order by trim(lower(dr))";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='conta_per_ruolo';
		$s.=get_alert('<h2>AB_CAN - Conta per ruolo</h2>','info');
		$s.='<br />';
		$s.='<div class="row">';
			$s.='<div class="col-sm-3">'.get_table_data($a[2],'','',$nt,'o',false).'</div>';	
			$s.='<div class="col-sm-9" id="contaR"></div>';	
		$s.='</div>';	
	} else {
		$s.=get_alert('<h2>Nessun dato</h2>','danger text-center');
	}
...
function get_conta_per_afferenza(){
	global $conn_new;
	$s='';
	$sql="select ka, da, count(ka) n from c##sss_import.ab_can group by ka, da order by trim(lower(da))";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='conta_per_afferenza';
		$s.=get_alert('<h2>AB_CAN - Conta per afferenza</h2>','info');
		$s.='<br />';
		$s.='<div class="row">';
			$s.='<div class="col-sm-3">'.get_table_data($a[2],'','',$nt,'o',false).'</div>';	
			$s.='<div class="col-sm-9" id="contaR"></div>';	
...
function get_conta_per_inquadramento(){
	global $conn_new;
	$s='';
	$sql="select ki, di, count(ki) n from c##sss_import.ab_can group by ki, di order by trim(lower(di))";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='conta_per_inquadramento';
		$s.=get_alert('<h2>AB_CAN - Conta per inquadramento</h2>','info');
		$s.='<br />';
		$s.='<div class="row">';
			$s.='<div class="col-sm-3">'.get_table_data($a[2],'','',$nt,'o',false).'</div>';	
			$s.='<div class="col-sm-9" id="contaR"></div>';	
...
function get_cyberguru_all(){
	global $conn_new;
	
/*
	$x=array();
	$x['A']=array();
	$x['A']["w"]		= "can.KR IN ('CO1L', 'CO2L', 'COCU5', 'COCU6', 'D2', 'D226')";
	$x['A']["info"]	=	"Allievi (A)";
	// $x['A']["cgrole"]	=	"Studente";
	$x['P']=array();
	$x['P']["w"]		= "can.GEST = 'UGOV'";
	$x['P']["info"]	=	"Docenti e PTA (P)";

	foreach (array_keys($x) as $xi){
		$sql = "SELECT DISTINCT id_ab, LOWER (ldap_uid) ldap_uid, LOWER (mail) mail, LOWER (NVL (SUBSTR (mail, 0, INSTR (mail, '@') - 1), mail)) utente, can.gest, INITCAP (NOME) NOME, INITCAP (COGNOME) COGNOME, can.COD_FISC CF, GENERE, KR, DR, r.RUOLO_NET7 dr_gr, KA, DA, a2l.BREVE da_2l, SUBSTR (can.COD_FISC, 12, 4) COMUNE, CASE WHEN it.AREA IS NOT NULL THEN it.AREA ELSE 'Estero' END area, CASE WHEN SUBSTR (can.COD_FISC, 12, 1) = 'Z' THEN 'en' ELSE 'it' END lingua, INITCAP (it.desc_comune) desc_comune,INITCAP (it.desc_provincia) desc_provincia, INITCAP (it.desc_regione) desc_regione, it.COD_NAZIONE, INITCAP (it.desc_nazione) desc_nazione, CASE WHEN it.cod_nazione IN ('AT', 'BE', 'BG', 'CY', 'HR', 'DK', 'EE', 'FI', 'FR', 'DE', 'GR', 'IE', 'IT', 'LV', 'LT', 'LU', 'MT', 'NL', 'PL', 'PT', 'CZ', 'RO', 'SK', 'SI', 'ES', 'SE', 'HU') OR it.cod_nazione IS NULL THEN 1 ELSE 0 END ue, TO_CHAR (DT_NASCITA, 'yyyy/mm/dd') DT_NASCITA, TO_CHAR (DT_NASCITA, 'yyyy') aa_nascita, TO_CHAR (SYSDATE, 'yyyy') - TO_CHAR (DT_NASCITA, 'yyyy') ETA FROM c##sss_import.AB_CAN can, c##sss_import.ab_ac_comuni_x it, c##sss_import.ab_ruolo_new r, (SELECT * FROM c##sss_import.ab_vista_org WHERE typ = 'org') a, (SELECT * FROM c##sss_import.ab_vista_org WHERE typ = 'org' AND livello = 2) a2l WHERE can.ldap_uid IS NOT NULL AND SUBSTR (can.COD_FISC, 12, 4) = it.codice_comune(+) AND can.kr = r.K_RUOLO(+) AND can.ka = a.uo(+) AND SUBSTR (can.rpath, 0, 14) = a2l.rpath(+) AND ".$x[$xi]['w']." ORDER BY cognome, nome"; 	
	}	
*/
	
	$s='';
	$sql="select * from c##sss_import.ab_cyberguru_all order by cognome, nome";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='cyberguru_all';
		$s.=get_alert('<h2>cyberguru Tutti</h2>','info');
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
		$s.=get_alert('<h2>Nessun dato</h2>','danger text-center');
	}
...
function get_cyberguru_doc_pta(){
	global $conn_new;
	$s='';
	$sql="select * from c##sss_import.ab_cyberguru_doc_pta order by cognome, nome";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='cyberguru_doc_pta';
		$s.=get_alert('<h2>cyberguru Docenti e PTA</h2>','info');
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
		$s.=get_alert('<h2>Nessun dato</h2>','danger text-center');
	}
...
function get_cyberguru_allievi(){
	global $conn_new;
	$s='';
	$sql="select * from c##sss_import.ab_cyberguru_allievi order by cognome, nome";
	if (!empty($_SESSION['IAM']['my_acl']['ict'])){
		if ($_SESSION['IAM']['my_acl']['ict']=='S'){
			$s.=get_alert($sql);
		}
	...
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='cyberguru_allievi';
		$s.=get_alert('<h2>cyberguru Allievi</h2>','info');
		$s.='<br />'.get_table_data($a[2],'','',$nt);
	} else {
		$s.=get_alert('<h2>Nessun dato</h2>','danger text-center');
	}
...
function get_ldap_da_disabilitare(){
	global $conn_new;
	$s='';
	$g=[['LDAP_UID','CODFIS'],['HA_PUNTO_IN_UID'],['COGNOME','NOME'],['UMAIL','TELEPHONENUMBER'],['DEPARTMENTNUMBER','OU'],['PHYSICALDELIVERYOFFICENAME'],['SCHACPERSONALPOSITION','BUSINESSCATEGORY','EMPLOYEETYPE','TITLE'],['CN','DESCRIPTION','SAMBAACCTFLAGS'],['FINE_RAPPORTO']];
	$d=['Uid Codice fiscale','ha .','Cognome Nome','Mail Telefono','Afferenza Organizzativa','Sede','Ruolo','cn Descrizione Samba','Fine Rapporto'];
	$campiLDAP="LDAP_UID, UIDNUMBER, SN COGNOME, GIVENNAME NOME, MAIL UMAIL, TITLE, TELEPHONENUMBER, CF CODFIS, EMPLOYEENUMBER, DEPARTMENTNUMBER, OU, PHYSICALDELIVERYOFFICENAME, BUSINESSCATEGORY, SCHACPERSONALPOSITION, ALIASEDOBJECTNAME, DESCRIPTION, EMPLOYEETYPE, SCADENZATEMPO, SAMBAACCTFLAGS, CN, TO_CHAR(TO_DATE (SUBSTR (NVL (SCADENZATEMPO, '19000101'), 1,8), 'yyyymmdd'),'yyyy/mm/dd') FINE_RAPPORTO, case when instr(ldap_uid, '.') > 0 then 1 else 0 end ha_punto_in_uid";
	$sql='';
	// $sql.="select ".$campiLDAP." from c##sss_import.anagrafiche_da_ldap, (select distinct ldap_uid ab_can_uid from c##sss_import.ab_can) a where ldap_uid = ab_can_uid(+) and ab_can_uid is null";
	// $sql.=" union ";
	$sql.="select ".$campiLDAP." from c##sss_import.anagrafiche_da_ldap";
	// $sql.=", (select distinct cod_fisc from c##sss_import.ab_can) a";
	// $sql.=" where cf is not null and cf = cod_fisc(+) and cod_fisc is null";
	$sql.=" where cf not in (SELECT DISTINCT cod_fisc FROM c##sss_import.ab_can)";
	$sql.=" order by trim(lower(sn || givenname))";
	...
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {	// almeno una riga e almeno una colonna
		$nt='ldap_da_disabilitare';
		$s.=get_alert('<h2>LDAP da disabilitare ('.$a[1].' records)<h2>','info text-center');
		$s.=get_alert('<h4>Questa ricerca visualizza gli account LDAP che fanno riferimento a carriere scadute</h4>','warning');
		$s.='<br />';
		if (!empty($_SESSION['IAM']['my_acl']['ict'])){
			if ($_SESSION['IAM']['my_acl']['ict']=='S'){
				$s.=get_alert($sql);
...
function crt_filter($s,$sok,$sp=false){
/*
...
*/
	...
	for ($i=0; $i<strlen($s); $i++){
		if (strpos($sc,$s[$i]) !== false){
			$out.=$s[$i];
		} else {
			if ($sp){$out.=' ';}	// se richiesto mette uno spazio al posto del carattere scartato
		}
	}
	return $out;
}
function ab_str_utf8_ascii($s){
	$as = str_split(convert_utf8($s));
	$t=''; $p=0; $tc='';
	foreach ($as as $c) {
		$n=ord($c);
		if ($n == 32 					// (spazio)
...
function get_can_d($uid=''){
	...
	$sql="select a.*, to_char(a.DT_RAP_FIN,'dd/mm/yyyy') DTF from c##sss_import.ab_can a where ldap_uid = '".$uid."'";	
	$a=load_db($conn_new,$sql,'o');
	if ($a[0]>0 and $a[1]>0) {
		...
		$s.='<div class="row py-2">'; // intestazione
			...
			$s.='<div class="col-sm-10">';
				$s.='<div class="row">';								
					$s.='<div class="col-sm-3 text-right">Nome e cognome:</div>';
					$ss=@htmlentities($a[2]['NOME'][0].' '.$a[2]['COGNOME'][0]);
					$s.='<div class="col-sm-9"><h3>'.$ss.'</h3></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Gestionale:</div>';
					$ss=@htmlentities($a[2]['GEST'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Uid:</div>';
					$ss=@htmlentities($a[2]['LDAP_UID'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Codice fiscale:</div>';
					$ss=@htmlentities($a[2]['COD_FISC'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Mail:</div>';
					$ss=@htmlentities(strtolower($a[2]['MAIL'][0]));
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Telefono interno:</div>';
					$ss=@htmlentities($a[2]['TEL'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Ruolo:</div>';
					$ss=@htmlentities($a[2]['KR'][0].' - '.$a[2]['DR'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Afferenza:</div>';
					$ss=@htmlentities($a[2]['KA'][0].' - '.$a[2]['DA'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Inquadramento:</div>';
					$ss=@htmlentities($a[2]['KI'][0].' - '.$a[2]['DI'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Fine rapporto:</div>';
					$ss=@htmlentities($a[2]['DTF'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
				$s.='<div class="row">';
					$s.='<div class="col-sm-3 text-right">Nota:</div>';
					$ss=@htmlentities($a[2]['NOTA'][0]);
					$s.='<div class="col-sm-9"><strong>'.$ss.'</strong></div>';
				$s.='</div>';
			$s.='</div>';
		$s.='</div>';
	}	else {
		$s.=get_alert($uid.' non trovato','danger');
	}
	return $s;
}
...
// if ($sviluppo_albo){write_log('_tmp.log',"ldap_authenticate - dopo ldap_connect (err: ".$o['err'].")");}		
		if ($ds) {
			...
			try {
				...
				if (ldap_errno($ds)){	// error number
					ldap_get_option($ds, LDAP_OPT_DIAGNOSTIC_MESSAGE, $err); // leggo dettagli errore
					$e=get_alert('AD '.ldap_error($ds).' ('.ldap_errno($ds).')<br>'.$err,'danger', true);
					$o['err'].=$e;
				}
			...
			if ($sr) {
				...
				for ($i=0; $i<$nen; $i++) {
					...
					if ($upn==$username.'@santannapisa.it') {
						...
						// restore_error_handler();
						try {
							$ldapBind = @ldap_bind($ds,$dn,$password);	// @@@@ albo 25/03/2024
							if (ldap_errno($ds)){	// error number
								ldap_get_option($ds, LDAP_OPT_DIAGNOSTIC_MESSAGE, $err); // leggo dettagli errore
								$e=get_alert('AD ldap_bind '.ldap_error($ds).' ('.ldap_errno($ds).')<br>'.$err,'danger', true);
								$o['err'].=$e;
							}
						} catch (Exception $e) {
							$o['err'].=get_alert('ERROR Message: ' .$e->getMessage(),'danger',true);
	...
	// if ($su and $o['err'] != ''){$o['err'].='<br />'.$md.' - u: <strong>'.$username.'</strong> - p: <strong>'.$password.'</strong> ';}
	return $o;
}
		...
		if (!empty($en[0])){
			if (is_array($en[0])){
				...
				for($j=0; $j<count($r0); $j++){	// loop sulle righe
					...
					if (!empty($en[0][$i][0])){
						$x='';
						if (in_array($i,array('xscadenzatempo','x-scadenzatempo','whencreated','whenchanged','dscorepropagationdata','mstsexpiredate'))){
							$s.=get_alert('<h4 class="mt-2 mb-0">'.$i.'</h4>','primary',true);
							for ($z=0; $z<count($en[0][$i]); $z++){
								$d=substr($en[0][$i][$z],0,8);
								$d=substr($d,6,2).'/'.substr($d,4,2).'/'.substr($d,0,4);
								if (!empty($en[0][$i][$z])){
									$s.=get_alert(($z+1).' - <strong>'.$en[0][$i][$z].'</strong> <span class="text-danger">(data) : '.$d.'</span>','info',true);
								}
							}
							$x='-';
						} 
						if (in_array($i,array('accountexpires','lastlogon','pwdlastset','badpasswordtime','lastlogontimestamp'))){
							$s.=get_alert('<h4 class="mt-2 mb-0">'.$i.'</h4>','primary',true);
							for ($z=0; $z<count($en[0][$i]); $z++){
								$d = $en[0][$i][$z];
								$winInterval = round($d / 10000000);
								$unixTimestamp = ($winInterval - 11644473600);
								if (!empty($en[0][$i][$z])){
									$s.=get_alert(($z+1).' - <strong>'.$en[0][$i][$z].'</strong> - <span class="text-danger">(data) : '.date("d/m/Y", $unixTimestamp).'</span>','info',true);
								}
							}							
							$x='-';
						}
						if (in_array($i,array('sambapwdlastset','sambapwdmustchange','sambakickofftime','sambalogofftime'))){
							$s.=get_alert('<h4 class="mt-2 mb-0">'.$i.'</h4>','primary',true);
							for ($z=0; $z<count($en[0][$i]); $z++){
								$unixTimestamp = $en[0][$i][$z];
								if (!empty($en[0][$i][$z])){
									$s.=get_alert(($z+1).' - <strong>'.$en[0][$i][$z].'</strong> - <span class="text-danger">(data) : '.date("d/m/Y", $unixTimestamp).'</span>','info',true);
								}
							}							
						...
						// campo vuoto
						$s.=get_alert('<h4 class="mt-2 mb-0">'.$i.'</h4>','warning',true);
					}
				}
			}
		}
...
function input_to_db($data, $inim=false) {
	// $prima=$data;
	$data = ab_str_utf8_ascii($data);
	$data = trim($data);
	$data = addslashes($data);
	if ($inim){
		// tutto minuscolo tranne le iniziali di ogni parola
		$data = strtolower($data);
		$data = ucwords($data);
	}
	// write_log('_debug.txt', 'input_to_db - prima: '.$prima."\ndopo: ".$data);
  return $data;
}
?>
